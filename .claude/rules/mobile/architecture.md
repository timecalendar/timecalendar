...
- `src/theme/tokens.ts` holds the design tokens as plain `as const` TypeScript: `Colors` (light/dark records), `Spacing`, the new `Radii` scale (radius is a token, not a magic number), `Fonts`, and the layout constants (`BottomTabInset`, `MaxContentWidth`); `ThemeColor` is derived from `Colors`. **No styling runtime** (NativeWind / Tamagui / unistyles rejected, R-2 — `StyleSheet` + typed token constants is the established pattern, and `tsc` stays the only type gate). A missing/mistyped token key is a `tsc` error, not a silent fallback.
- `src/theme/use-theme.ts` owns light/dark resolution: `useTheme()` reads the `@/hooks/use-color-scheme` wrapper (kept — it's scheme *detection*, not tokens, and carries the web static-render fix) and returns `Colors[scheme]`. The app follows the device scheme; **no in-app theme override** (a Settings / MMKV-seam concern, deferred like the i18n language switcher).
- `src/theme/index.ts` re-exports the public surface (`Colors`/`Spacing`/`Radii`/`Fonts`/`BottomTabInset`/`MaxContentWidth`/`ThemeColor`/`useTheme`) so **call sites import `@/theme`**, and carries the **`@/global.css` side-effect import** that travelled out of the deleted `constants/theme.ts` (it loads the web font CSS variables the `Fonts.web` tokens reference — every token consumer imports `@/theme`, so it loads with the tokens; dropping it breaks web font rendering). The old `constants/theme.ts` and `hooks/use-theme.ts` are **deleted**.
- `Themed*` and every other consumer were repointed at `@/theme` **import-source-only**. `ThemedText`'s heading-role contract (a11y step 7: `type="title"|"subtitle"` → `accessibilityRole="header"`, caller override wins) is **unchanged** and its `themed-text.test.tsx` proof passes unmodified — that test is the regression guard for the repoint (D2).
...
### What CI proves vs. what's manual (D7)
- `src/theme/theme.test.tsx` (mirroring the i18n/a11y/firebase proofs, gated by `test-mobile`: tsc + lint + Jest, R-1) asserts the two things tooling *can*: (a) `useTheme` resolves a token to its expected **light** value and its expected **dark** value (driving `useColorScheme`'s two states via a mock of `@/hooks/use-color-scheme`) — light/dark resolution end to end, not merely that the constants exist; (b) `GlassSurface` renders its children in Jest, where `isLiquidGlassAvailable()` is `false`, exercising the **fallback** path (degrades to a real `View`, not a throw).
