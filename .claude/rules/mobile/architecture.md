...
- **Single color-scheme seam (C1, `drop-mobile-web-and-brand-theme` D3):** `@/hooks/use-color-scheme` (`export { useColorScheme } from "react-native"`) is the **one** scheme source — both `useTheme` *and* the root `_layout.tsx` read it (the layout previously imported `useColorScheme` straight from `react-native`, a second source). Routing both through the wrapper means Settings' future light/dark/system override is a single-file change. The wrapper is a pass-through today; the value is the seam, not present behavior. (The web static-render `.web.ts` variant was deleted with the web target — Metro now resolves the bare `.ts` on both platforms.)
...
- **`chrome/index.ts`** — the barrel. Exports `NativeTabs` and `GlassSurface`, and carries a **documented `@expo/ui` boundary-only note**: `@expo/ui` gets a lint boundary, **not** a rendered stub (D6, R-2) — it has no consumer, and a throwaway control "to prove the wrapper" would be exactly the speculative divergence R-2 forbids. When the first consumer arrives (likely Settings, Phase 1.5) an `expo-ui.tsx` wrapper joins the seam. The boundary is real and enforced now; the body is earned by its first user.
