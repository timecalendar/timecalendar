...
// build queries with, and the reactive read hook the storage section names as
...
// drizzle-orm/sqlite-core import stays inside the seam dir. The repositories look
// events up by uid with `eq`; the event-checklists ordered read uses `asc` (ADR
// 024 — `order BY order` asc; R-2: re-export only what a consumer needs).
export {
  calendarEvents,
  checklistItems,
  personalEvents,
