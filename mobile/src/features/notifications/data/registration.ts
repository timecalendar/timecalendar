...
// Android device-language change while the pref is "system") and a device
// timezone change (reactive via expo-localization) each re-PUT so the server
// renders in the new locale/zone. Every cold start PUTs the full DTO, which
// backstops any change the live triggers miss.
//
...
export function useNotificationRegistration(): void {
  ...
  const timezone = useCalendars()[0]?.timeZone ?? null
