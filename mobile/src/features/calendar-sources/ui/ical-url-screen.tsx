...
// The iCal-URL entry screen (Phase-3 ship 4 / design D6) — PRESENTATIONAL (70%
// floor): a labeled RN-core TextInput for the calendar URL, a submit control, and
// accessible loading / server-error-with-retry states over the create mutation
// (mirroring school-selection's read flow per data.md). It posts the URL to the
// server (POST /calendars), not a client-side .ics parse — Flutter parity (D1).
//
...
export default function IcalUrlScreen() {
  ...
  const { createCalendar, isPending, isError, reset } = useCreateCalendar()
  ...
  const submit = () => {
    ...
    setErrorKey(null)
    reset()
    void createCalendar(url)
      .then(() => {
        setScannedSource({ url: url.trim() })
        router.back()
      })
      .catch((error: unknown) => {
        // Genuine server failure — record through the seam, surface a11y error.
        recordError(
