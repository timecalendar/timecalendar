...
export function ImportDraftProvider({ children }: { children: ReactNode }) {
  ...
  const setListedInstitution = useCallback(
    (school: SchoolListItem) => dispatch({ type: "set-listed", school }),
    [],
  )
  const setUnlistedInstitution = useCallback(
    (schoolName: string) => dispatch({ type: "set-unlisted", schoolName }),
    [],
  )
  const setCalendarName = useCallback(
    (name: string) => dispatch({ type: "set-calendar-name", name }),
    [],
  )
  const clearDraft = useCallback(() => dispatch({ type: "clear" }), [])
  const seedDevelopmentCompletion = useCallback(
    (draft: CalendarImportDraft, snapshot: ExportGuidePinnedSnapshot) => {
