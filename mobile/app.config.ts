...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  // EAS Build/Submit seam (add-mobile-eas). The real projectId was produced by
  // `eas init` and is committed here as the fallback — it is NOT a secret (it
  // ships in the binary and the EAS project is public-by-id). EAS_PROJECT_ID can
  // override it. xprem delivery is configured independently below. See eas.md.
  const easProjectId =
    ...
    updates: isDev
      ...
      : {
          ...
          requestHeaders: {
            ...
            // Empty means xprem's channel mapping decides the branch.
            "xprem-branch": "",
          },
        },
  }
