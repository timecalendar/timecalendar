...
- [ ] 1.1 From `mobile/`, run `npx expo install expo-application` so
...
- [ ] 1.2 Set `mobile/app.config.ts` version to `4.0.0`; verify development and
...
- [ ] 2.1 Create `mobile/src/features/about/{data,ui}/` with sublayer barrels and a
  minimal feature public barrel; verify the module follows the existing feature boundary
  and import-order lint rules.
- [ ] 2.2 Add the narrow `expo-application` metadata reader and a pure total derivation
...
- [ ] 3.1 Export `SettingsSection`/`SettingsRow` through the Settings UI barrel and
...
- [ ] 3.2 Implement `AboutScreen` with a safe-area-aware themed scroll surface, two short
...
- [ ] 3.3 Add typed EN/FR catalog keys for the screen title, natural two-paragraph copy,
...
- [ ] 3.4 Add `about-screen.test.tsx` with mocked native seams and explicit EN/FR cases;
...
- [ ] 4.1 Add the one-line `mobile/src/app/about.tsx` UI-barrel export, register About as
  ...
  anchor.
- [ ] 4.2 Add an explicit third `app` section after Events and Preferences in the
...
- [ ] 4.3 Extend `mobile/src/components/settings-route-structure.test.ts` as the CI proof
...
- [ ] 5.1 Add `mobile/.maestro/about.yaml`: clear state, cold-open
...
- [ ] 5.2 Update `docs/mobile/architecture-book/features.md` with standalone About
...
- [ ] 5.4 Add a dated `docs/react-native-migration/inbox/` note tagged for a human pass
...
- [ ] 6.1 From `mobile/`, run `npx tsc --noEmit`, `npm run lint`, Prettier checks on all
...
- [ ] 6.2 Run `openspec validate add-mobile-about-screen`, inspect the final diff for
