...
a custom health module. Three sibling services — **sibling-service-a** (most extensive),
**sibling-service-b**, and **sibling-service-c** — already stand on nest-shared and share one mental
...
- **Adopt `SharedRedisModule` + `SharedBullModule`.** Replace the custom
  ...
  sibling-service-c/sibling-service-b/sibling-service-a. The existing `@nestjs/bullmq` processors and the
...
- **Keep TimeCalendar's richer bootstrap; adopt only the filter from it.** nest-shared's
  ...
  `TypeOrmExceptionFilter` — mirroring sibling-service-a's hybrid approach (design D5).
