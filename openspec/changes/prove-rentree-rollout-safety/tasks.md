...
- [x] 1.1 Change only the frozen backfill in
...
- [x] 1.2 Add a PostgreSQL-backed migration integration test using the existing isolated
...
- [x] 1.3 Run the real `up → down → up` procedure on a representative non-production
...
- [x] 2.1 Add a `CalendarRepository` compare-and-set method that advances
...
- [x] 2.2 Update `CalendarSyncService.sync` so an existing calendar resolves its strategy
...
- [x] 2.3 Extend focused sync tests for a generic 30-minute plan, Lyon 1 60-minute plan,
...
- [x] 2.4 Update the existing background-sync and sync-policy tests/spec assumptions that
...
- [x] 3.1 Create `docs/server/rentree-release-runbook.md` with copy/paste commands to
...
- [x] 3.2 Add preproduction verification commands for platform desired tags, Argo CD
  ...
  target.
- [x] 3.3 Document the representative `up/down/up` procedure and exact read-only lock,
...
- [x] 4.1 Add statement-time-limited, read-only SQL to the runbook for total and 14-day
...
- [x] 4.2 Add commands/queries that capture ready server replicas, each pod's actual
...
- [x] 4.3 Add a worksheet that calculates nominal concurrency, jobs-per-five-minute
...
- [x] 4.4 Record a sanitized production-safe baseline using the committed aggregate
...
- [x] 5.1 Add the ordered preproduction soak: candidate convergence, migration evidence,
...
- [x] 5.2 Add the production go/no-go checklist and observation windows without changing
...
- [x] 5.3 Document normal emergency rollback as restoring the previously recorded
...
- [x] 6.1 Evaluate the Architecture Book against the implemented change. Because this is
...
- [x] 6.2 Update the existing server calendar sync specs/tests and link the release runbook
  from the relevant production-health investigation. Do not add implementation chronology
  to the mobile Architecture Book.
- [x] 6.3 Verify `openapi/openapi.json` and `mobile/src/api/generated/` remain unchanged.
  Any generated contract drift is a defect and must be resolved rather than accepted.

## 7. Local-green and CI proof

- [x] 7.1 Run the focused migration, calendar repository, calendar sync service/all, fan-out,
  ...
  retry behavior.
- [x] 7.2 Run `cd server && npx tsc --noEmit` and `cd server && npm run lint`; inspect and
  commit only formatter/lint changes within this issue's server/docs/OpenSpec scope.
- [x] 7.3 Run `cd server && npm run generate:openapi`, then verify both
  `openapi/openapi.json` and `mobile/src/api/generated/` have no diff. Run
  `openspec validate prove-rentree-rollout-safety`.
- [x] 7.4 Inspect the final diff and history for secrets, certificates, customer data, raw
