...
- [ ] 1.1 Add a focused dependency-free check under `ci/` that scopes assertions to the
...
- [ ] 1.2 Add focused negative fixtures or self-tests for any nontrivial block parser used by the
  check, proving that matching words in comments or in the other package's step cannot satisfy an
  invariant.
- [ ] 1.3 Wire the focused check into an always-on, non-deploying baseline job in
...
- [ ] 2.1 Give `clean-ghcr` only job-level `packages: write`; remove the stored PAT reference and
...
- [ ] 2.2 Add a separate, visibly named v2 invocation for the exact web package name with the same
  workflow-token mode, organization account inputs, and cut-off expression; do not combine names or
  use a wildcard.
- [ ] 2.3 In both package steps preserve `keep-at-least: 5` and `skip-tags: latest, production`, and
...
- [ ] 2.4 Re-run the focused check and validate YAML syntax and v2 action input names with a pinned
...
- [ ] 3.1 Update `docs/agent-dev-environment.md` to describe the repository-token authentication,
...
- [ ] 3.2 Complete the Architecture Book/ADR assessment: because this changes repository automation
...
- [ ] 3.3 Re-read the diff for sensitive-surface scope: only the retention workflow, its baseline CI
...
- [ ] 4.1 Run the focused retention check and its negative/self-test cases, and confirm the valid
  workflow passes without contacting GitHub or loading secrets.
- [ ] 4.2 Run `openspec validate repair-ghcr-retention-authentication --strict` and update these
  checkboxes and artifacts to match the implementation that actually landed.
- [ ] 4.3 Run the repository disclosure scan over the complete branch and exact PR text using the
