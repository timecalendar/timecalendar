...
- [ ] 1.1 From `mobile/`, record `npx eoas@3.1.2 --version`, snapshot the clean config diff, then
...
- [ ] 1.2 Normalize the initializer output in `mobile/app.config.ts`: keep the locked manifest URL,
...
- [ ] 1.3 Add a validated `OTA_CHANNEL` build-time contract: `APP_VARIANT=development` disables
...
- [ ] 2.1 Configure release `updates.requestHeaders` with `expo-channel-name` from the validated
...
- [ ] 2.2 Set `OTA_CHANNEL=preview` and `OTA_CHANNEL=production` in the matching
...
- [ ] 2.3 Preserve `extra.eas.projectId` as `EAS_PROJECT_ID` with committed fallback
...
- [ ] 3.1 Configure release `updates.codeSigningCertificate` as
...
- [ ] 3.2 Validate the committed PEM with `openssl x509` and confirm SHA-256 fingerprint
...
- [ ] 3.3 Harden `mobile/.gitignore` for private-key directories and common private
...
- [ ] 4.1 Add a focused CI-discovered config test that resolves isolated development, preview, and
...
- [ ] 4.2 Extend the focused proof to recursively reject every `channel` key in `mobile/eas.json`
...
- [ ] 4.3 Run the focused test directly, then run production/preview/development
...
- [ ] 5.1 Using the SDK-56 project-local command
...
- [ ] 5.2 In a `PAPERCLIP_RUN_SCRATCH_DIR` copy, add one harmless, documented native-config probe,
...
- [ ] 5.3 If and only if preview/production differ for a safely isolatable channel-only file,
...
- [ ] 6.1 Create separate run-owned scratch copies for preview and production, reuse the installed
...
- [ ] 6.2 Attempt the smallest EAS profile validation for preview and production without exposing
...
- [ ] 7.1 Update `docs/mobile/architecture-book/eas.md` from deferred to implemented: document the
...
- [ ] 7.2 Extend ADR 037 (not onboarding ADR 036) with Decision sections for the concrete client
...
- [ ] 7.3 Update `mobile/EAS.md`, the Architecture Book `CHANGELOG.md`, and Phase 10 roadmap step 5
...
- [ ] 8.1 Run `npx tsc --noEmit`, `npm run lint`, and Prettier check/format for every touched
      mobile/config/Markdown file; run `git diff --check`
- [ ] 8.2 Run `npm test -- --coverage` so the focused config proof executes in the same CI posture
      ...
      ticket
- [ ] 8.3 Run `openspec validate wire-mobile-xprem-signed-channels --strict` and
