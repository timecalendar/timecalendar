...
Pressing `personal-event-delete` will call `Alert.alert` with a localized title and body that explicitly name permanent event deletion. The button array will place a localized Cancel action first with `style: "cancel"` and a localized Delete action second with `style: "destructive"`. Android alert options will allow native back/outside dismissal and use `onDismiss` only to return the local prompt guard to idle; it will never write. React Native exposes no corresponding iOS dismissal callback, so iOS retains the guard through the synchronous presentation edge and then releases it while the system modal owns interaction exclusion. A generation token admits actions only from the latest alert. VoiceOver escape therefore needs no unavailable callback to make Delete reopen, while a later prompt safely supersedes any older action closure.
...
The component suite will spy on `Alert.alert`, press `personal-event-delete`, and invoke the captured cancel/destructive callbacks plus Android's captured `onDismiss`. The iOS proof will deliberately receive no `onDismiss`: it settles the presentation edge, represents native VoiceOver escape, and proves the form can reopen a later alert without a write. Tests will also prove that confirm passes the uid; repeated destructive callbacks while a deferred promise is pending produce one write; success navigates once; failure does not navigate, preserves the visible error, releases the guard, and permits a successful retry.
