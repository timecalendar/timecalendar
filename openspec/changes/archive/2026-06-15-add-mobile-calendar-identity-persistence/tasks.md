...
- [x] 1.1 Add a `userCalendars` `sqliteTable` (SQL table name `user_calendars`, the sembast
...
- [x] 2.1 Run `npm run generate:migrations` in `mobile/`. Confirm it emits a new
...
- [x] 2.2 Confirm `src/db/migrations/migrations.d.ts` still types the regenerated bundle (stable
...
- [x] 3.1 Re-export the `userCalendars` table from `src/db/index.ts` (`export { personalEvents,
...
- [x] 4.1 `types.ts` — the domain `UserCalendar` type (`id`, `token`, `name`,
...
- [x] 4.2 `id.ts` — `newId()` over `expo-crypto`'s `randomUUID` (the single import site,
  swappable; mirrors `personal-events/data/uid.ts`). The importer bypasses it by supplying its
  own recovered id to `upsert`.
- [x] 4.3 `repository.ts` — async CRUD over `@/db` (imports `{ db, userCalendars, eq }` only;
...
- [x] 4.4 `hooks.ts` — `useUserCalendars()` over `useLiveQuery(db.select().from(userCalendars))`
  (re-exported from `@/db`, never a direct `drizzle-orm` import), mapping live rows → domain.
- [x] 4.5 `add-calendar.ts` — the SHARED persist seam (design D6): `addCalendarFromUrl(url)` (or
...
- [x] 4.6 `index.ts` — the `user-calendars` sub-module barrel re-exporting the public surface
...
- [x] 5.1 Delete `src/features/calendar-sources/data/scanned-source.ts` and its test
...
- [x] 5.2 `ui/qr-scan-screen.tsx` — on a successful parse, call `addCalendarFromUrl(source.url)`
...
- [x] 5.3 `ui/ical-url-screen.tsx` — replace the `setScannedSource({ url })` success branch with
...
- [x] 5.4 Update any onboarding screen that read `useScannedSource` (the "calendar added"
...
- [x] 6.1 Add any new user-facing string the persist-failure path needs (e.g.
...
- [x] 7.1 `data/user-calendars/types.test.ts` — the mappers: round-trip (domain → row → domain,
  ...
  `visible: true`).
- [x] 7.2 `data/user-calendars/id.test.ts` — `newId()` delegates to the mocked
  `expo-crypto.randomUUID`.
- [x] 7.3 `data/user-calendars/repository.test.ts` — each function's Drizzle query SHAPE +
...
- [x] 7.4 `data/user-calendars/restart.test.ts` (the central durability proof) — drive the
  ...
  proves the contract.
- [x] 7.5 `data/user-calendars/add-calendar.test.ts` — the persist wiring: success (mock the
  ...
  (testing.md).
- [x] 7.6 Update the rewired screen tests (`ui/qr-scan-screen.test.tsx`,
...
- [x] 7.7 Run `npm test -- --coverage`; the new logic paths (`src/db/**`,
...
- [x] 8.1 Determine whether a durable calendar is reliably assertable after `stopApp` /
...
- [ ] 8.2 N/A — no stable post-relaunch assertion target exists: a "your calendars" list UI is an
  explicit Non-Goal this ship, and the onboarding welcome screen renders no "N calendars added"
  indicator from the reactive read, so a Maestro `stopApp`/`launchApp` flow would have nothing
  deterministic to assert. 8.3 lands instead.
- [x] 8.3 (HUMAN: see `inbox/2026-06-16-calendar-restart-durability.md`) — wrote the inbox note
  specifying the manual restart / kill / JS-cache-clear on-disk durability pass on iOS + Android
  (the SQLite-inspection how-to, since no UI reads the row this ship). Exactly one of 8.2 / 8.3
  landed (8.3) — the E2E axis is not silently skipped.

## 9. Architecture Book + ADR + roadmap

- [x] 9.1 Write **ADR 018** `.claude/rules/mobile/decisions/018-user-calendar-storage.md` (copy
...
- [x] 9.2 Add the ADR 018 row to `.claude/rules/mobile/decisions/README.md` index.
- [x] 9.3 Add a `user_calendars` schema section to `.claude/rules/mobile/storage.md` (mirroring
...
- [x] 9.4 Update `.claude/rules/mobile/features.md` — the calendar-sources entry: the durable
...
- [x] 9.5 Append an entry to `.claude/rules/mobile/architecture-changelog.md` (date · change-slug
  · what moved · why · pointer).
- [x] 9.6 Tick step 5 in `docs/react-native-migration/01-roadmap/03-onboarding-and-sources.md`.

## 10. Verify

- [x] 10.1 `npx tsc --noEmit` clean; `npm run lint` (`--max-warnings 0`) clean; `npm test --
  --coverage` green with the K-3 gate satisfied.
- [x] 10.2 `openspec validate add-mobile-calendar-identity-persistence --strict` passes.
- [x] 10.3 Confirm no `useScannedSource` / `setScannedSource` / `scanned-source` references
