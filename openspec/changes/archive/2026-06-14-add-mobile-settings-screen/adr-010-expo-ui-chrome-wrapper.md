...
> DRAFT for the `add-mobile-settings-screen` change (TIM-131 / A2). On apply, the
> implementer copies this to
> `.claude/rules/mobile/decisions/010-expo-ui-chrome-wrapper.md` and adds the
> README index row (tasks §10.1). Origin: design D4 + D8. The full wiring lives in
> the Architecture Book "Theming & native-chrome"; this is the load-bearing decision.
...
1. **Adopt `@expo/ui` behind a chrome wrapper:** `src/components/chrome/expo-ui.tsx` is the single
   ...
   wrapper stays **thin**: it re-exports the universal `Host` + `Picker` (with `Picker.Item`
   re-attached), and **does not** bake in higher-level composed components or force theming onto the
   OS-chromed controls (a native picker adopts the platform's own appearance — forcing colors is the
   LCD laziness R-2 rejects and breaks the R-3 native-reference posture). Higher-level composition is
   earned by a second consumer / the Phase-1.5 golden-path exemplar.
