...
- [x] 1.2 Review the exact current workflow, ADR 043, ADR 044, `mobile/app.config.ts`, and the
...
- [x] 2.1 In `.github/workflows/ci-mobile-e2e.yml`, add
...
- [x] 2.2 Add the same step-local value beside `APP_VARIANT: development` in iOS's
...
- [x] 2.3 Complete the Architecture Book update/disposition: verify ADR 043 still owns the
...
- [x] 3.1 Run a focused static workflow check that proves exactly four
...
- [x] 3.2 From `mobile/`, run the focused `app.config.test.ts` suite and direct
...
- [x] 3.3 Run the repository's YAML/Markdown formatting checks for touched files,
...
- [x] 4.1 Run `openspec validate wire-pr-273-e2e-backend-capability --strict`, archive with
