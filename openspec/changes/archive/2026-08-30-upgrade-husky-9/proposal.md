...
- **New CI hygiene job** (`ci/test-git-hooks.sh` + a `test-hooks` job in `ci-build-deploy.yml`)
  ...
  reference **outside `openspec/changes/` and the check script itself**. This makes the two
  silent-regression gates permanent instead of review-time-only. Both path exclusions are
  load-bearing: this change's own artifacts quote the helper path (and archiving carries them onto
  `main` for good), and so does the check script, which has to name the string it bans in its
  failure message (tasks 5.6, 6.1).
...
- **Host-wide side effect:** the first v9 `prepare` flips `core.hooksPath` from `.husky` to
  `.husky/_` in the main checkout's **shared** `.git/config`, for all 49
