...
- [x] 1.2 Add `server/src/generate-openapi.ts` (sibling-service-b pattern): `NODE_ENV=test`, `NestFactory.create(AppModule)` without listening, write pretty-printed `openapi/openapi.json`, `app.close()`, exit non-zero on failure
