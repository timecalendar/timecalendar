...
- [x] 1.1 Confirm the checkout, remote head, and GitHub head are PR #273's existing named branch;
...
- [x] 1.2 Review the prior exact-head native failure, `mobile/.maestro/environment-switch.yaml`,
...
- [x] 2.1 In `mobile/.maestro/environment-switch.yaml`, add exactly one `scrollUntilVisible`
      ...
      `timeout: 60000`.
- [x] 2.2 Preserve the existing `extendedWaitUntil`, its selector and 60000 ms timeout, the
...
- [x] 3.1 Verify this remains a leaf-level Maestro driveability repair under R-4: no product or
...
- [x] 3.2 Confirm the change introduces no credential, device-install, or console-registration
...
- [x] 4.1 Parse the target YAML and run a focused structural assertion proving exactly one
      ...
      touched files.
- [x] 4.2 Run `./mobile/e2e/test_ci_mobile_e2e.sh` and a focused count/scope assertion proving
...
- [x] 4.3 Run `openspec validate scroll-pr-273-environment-switch-control --strict`,
...
- [x] 5.1 Archive the completed one-off change with
