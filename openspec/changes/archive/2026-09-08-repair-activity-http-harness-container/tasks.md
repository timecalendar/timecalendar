...
- [x] 5.3 Push the implementation and confirm the repository's existing server build/test and disclosure checks are green on the exact PR head. Treat the focused PostgreSQL regression and completed full-scale command as the CI/local proof for this repair; do not add or modify a workflow.
- [x] 5.4 Record the post-merge obligation for [TIM-522](/TIM/issues/TIM-522): prior candidate-bound evidence is invalid, so its release-readiness owner must freeze a new full SHA and rerun all affected head-bound evidence after this repair merges.
