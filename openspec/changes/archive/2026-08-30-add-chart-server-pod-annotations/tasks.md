...
- [x] 1.1 Before editing anything, materialise the `origin/main` chart and the real
  ...
  out locally at `$HOME/projects/perso/platform`; fetch it so the values come from
  ...
  PLAT=$HOME/projects/perso/platform
  git -C "$PLAT" fetch origin && git -C "$PLAT" rev-parse --short origin/main
  for e in preprod production; do
    git -C "$PLAT" show "origin/main:kubernetes/clusters/<cluster>/20-apps/timecalendar-$e/values.yaml" > "$S/$e.yaml"
