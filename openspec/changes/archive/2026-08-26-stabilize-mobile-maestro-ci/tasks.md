...
- [x] 1.1 Refactor `mobile/e2e/run_e2e.sh` so it boots `ci/e2e-server.sh` once,
      ...
      Bash 3.2.
- [x] 1.2 Add an explicit bounded startup-attempt argument whose default is one. Capture each
...
- [x] 1.3 Add a focused shell proof for the harness control flow using fake Maestro results:
...
- [x] 2.1 In both jobs in `.github/workflows/ci-mobile-e2e.yml`, install Maestro with
...
- [x] 2.2 Bound Android `:app:assembleRelease` at 3072 MiB heap, 1024 MiB Metaspace, two
...
- [x] 2.3 Make iOS simulator selection deterministic from `simctl` JSON and print
...
- [x] 2.4 Replace the workflow's four whole-suite retries with one harness invocation using at
...
- [x] 2.5 Run the focused shell proof as a CI step before native execution, and add static
...
- [x] 3.1 Add ADR 038 under `docs/mobile/architecture-book/decisions/` with `## Decision`
      ...
      `decisions/README.md`.
- [x] 3.2 Update `docs/mobile/architecture-book/testing.md` to replace the one-directory-
...
- [x] 3.3 Update `mobile/e2e/README.md` and `docs/agent-dev-environment.md` with the exact
      ...
      native proof.
- [x] 3.4 Confirm the documentation introduces no human-only credential/device/console step;
      therefore no `docs/react-native-migration/inbox/` note is needed.

## 4. Local-green verification

- [x] 4.1 Run `bash -n mobile/e2e/run_e2e.sh` and the focused harness shell proof; run
      ShellCheck if it is installed, recording an explicit N/A otherwise.
- [x] 4.2 Validate the workflow syntax and static invariants (exact Maestro pin in both jobs,
...
- [x] 4.3 Run `openspec validate stabilize-mobile-maestro-ci` and the repository's formatting
...
- [x] 4.4 Review `git diff --check` and the complete diff: only the CI workflow, mobile E2E
