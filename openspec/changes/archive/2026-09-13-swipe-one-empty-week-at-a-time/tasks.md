...
- [x] 1.4 Extend the Calendar screen suite to prove that accepted paging commits the selected date, page position, native month/year title, Agenda range, renderer generation, and accessible canvas context to one week; also prove stale completion cannot relabel the screen and retained T01 mount/Agenda/details/Today behavior remains usable, then run every edited Calendar suite.
...
- [x] 3.3 Keep no more than one pending replacement generation, release superseded generation state/timers/completions, and rebuild the same three slots around an accepted anchor at a stable cumulative page position without a post-commit transform jump, blank, or partial settled frame; expose only content-free test instrumentation needed to assert bounded retention and frame-coherent geometry.
...
- [x] 4.1 Wire swipe settlements into the screen/controller transition path so the old selected date, native title, and canvas label remain committed during held motion and an accepted revision updates page/date/context/Agenda range together; verify month/year title changes and stale-result rejection in the screen suite.
- [x] 4.2 Add previous-week and next-week increment/decrement actions to the adjustable owned canvas with typed French/English strings; make both use the same one-page request/settle path as gestures, render no visible arrow toolbar, and verify catalog parity and semantics.
...
- [x] 6.3 Record content-free evidence from any available iOS or Android runtime with exact build/device/OS and active refresh rate; when a current runtime or a required field is unavailable, mark held-drag, settle, fast-fling, reversal/interruption, repeated actions, reduced-motion, settled-announcement, frame continuity, and retained-page/generation observations pending for owner QA rather than turning automated results or incomplete diagnostics into native claims.
