...
- [x] 1.1 Confirm platform PR `lyrolab/platform#79` is merged and both
  `kubernetes/clusters/<cluster>/20-apps/timecalendar-production/values.yaml`
...
- [x] 2.1 Add `timecalendar.queueConcurrency: 100` to
  `k8s/timecalendar/values.yaml` with a comment tying the default to
  `server/src/config/constants.ts`.
- [x] 2.2 Add `QUEUE_CONCURRENCY` near the Redis block in
...
- [x] 3.1 Add a focused chart-render test under `ci/` that runs `helm template` and
  ...
  `"100"` and never `""`.
- [x] 3.2 Wire the focused chart-render test into `.github/workflows/ci-build-deploy.yml`
...
- [x] 4.2 Update `docs/agent-dev-environment.md` only if the new chart CI command needs
...
- [x] 5.1 Run `helm lint k8s/timecalendar` and the committed chart-render CI proof; both
  must pass locally.
- [x] 5.2 Render the chart against the exact platform production values and the exact
  platform preproduction values at `origin/main`; confirm each server ConfigMap contains
  exactly one `QUEUE_CONCURRENCY: "10"`.
- [x] 5.3 Run `helm template k8s/timecalendar` with no override and with
...
- [x] 5.4 Render the server ConfigMap from `origin/main` and from the implementation with
...
- [x] 5.5 Run `openspec validate render-queue-concurrency-from-chart` and record all exact
  commands/results in the PR and handoff as Reviewer evidence. Human/device QA is N/A.

## 6. Scope and sensitive-surface audit

- [x] 6.1 Inspect the final diff and confirm no checksum annotation, platform values,
...
- [x] 6.2 Confirm `k8s/` and the narrowly scoped CI workflow/test are the only sensitive
