...
- [x] 1.1 Add pure projection tests for an ordinary log and oversized old/new/changed arrays: pin
...
- [x] 1.2 Add cursor-codec tests that retain every version 1 decode case and cover version 2
...
- [x] 1.3 Mutation-check the focused tests by changing one fragment boundary and one resume offset;
...
- [x] 2.1 Add a v1-only projection helper that maps a source row into one ordinary item or stable
...
- [x] 2.2 Extend the cursor model to decode legacy version 1 cursors and issue version 2 next-position
...
- [x] 2.3 Extend the repository page seam with the inclusive anchored-row shape used only by a
...
- [x] 2.4 Update `CalendarLogService.searchV1` to consume a prefix of the virtual stream, return at
...
- [x] 2.5 Preserve unread counting as one first-page source-row count, empty-token behavior, all
...
- [x] 3.1 Extend the v1 controller/service suites to traverse multi-page fragments and assert source
...
- [x] 3.2 Add a focused React Native repository/coordinator regression using current DTOs: seed a
...
- [x] 3.3 Run the unversioned controller tests and add a regression around an oversized source log to
...
- [x] 4.1 Extend the deterministic PostgreSQL-backed HTTP test to seed the existing 3,656-change
...
- [x] 4.2 Extend the candidate harness's many-change measurement to include every page in that
  ...
  output schema.
- [x] 4.3 Mutation-check the HTTP proof by temporarily removing the 900,000-byte stop and shifting
...
- [x] 5.1 Add the next available Architecture Book ADR for virtual Activity fragments, stable ids,
...
- [x] 5.2 Update `docs/react-native-migration/05-tech-specs/activity-capacity-gate.md` with the current
...
- [x] 5.3 Review `architecture.md`, `storage.md`, `testing.md`, `definition-of-done.md`, and the
...
- [x] 6.1 Run the supported server OpenAPI generator with isolated local Postgres/Redis, then the
...
- [x] 6.2 Run focused server Jest for cursor, mapper/projection, repository, service/controller, legacy
...
- [x] 6.3 Run `openspec validate bound-activity-v1-serialized-pages`, `git diff --check`, and the
