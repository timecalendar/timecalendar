...
- `mobile-architecture-book`: Ratify the self-hosted OTA architecture in ADR 037 and use the repository's existing `CHANGELOG.md` as the rule-change log.
...
- Architecture documentation: `architecture.md`, ADR 037, the ADR index, `eas.md`, `firebase.md`, and `CHANGELOG.md`. `runtime.md` changes only if implementation reveals a reusable runtime contract not already owned by EAS/Firebase guidance.
