...
> `sibling-service-c`, `sibling-service-b`, `sibling-service-a`, the `nest-shared` source, and the TimeCalendar
...
Pin a caret range matching the latest the siblings run (`^1.10.x`, sibling-service-c's version).
...
**nest-shared's model** (identical across sibling-service-c/sibling-service-b/sibling-service-a): `setupTestDatabase()`
...
`EntityNotFoundError` → 404 job less idiomatically). This mirrors **sibling-service-a's** hybrid
...
benefit (the siblings themselves diverge here: sibling-service-b uses `configureApp`, sibling-service-a and
sibling-service-c do not).
