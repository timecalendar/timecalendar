...
- [ ] 1.1 Resolve the exact candidate commit and immutable server image exercised in preproduction;
...
- [ ] 1.2 Prove the candidate server is the image actually running in preproduction before gathering
...
- [ ] 2.1 Start the isolated non-production server dependencies, migrate, seed the deterministic
...
- [ ] 2.2 Exercise the candidate `POST /v1/calendar-logs/search` HTTP route over the same cohort shapes
...
- [ ] 2.3 Capture synthetic `EXPLAIN (ANALYZE, BUFFERS)` evidence for first/following pages and recent/
...
- [ ] 2.4 Run representative concurrent candidate-route reads and record request count/error rate,
  ...
  call counts.
- [ ] 2.5 Complete every readiness-table gate with a measured value and `PASS`/`FAIL`, including G7's
...
- [ ] 3.1 Inventory all Activity server/mobile telemetry sinks and their label/attribute keys:
...
- [ ] 3.2 Run the server metrics/controller/sanitizer and mobile coordinator/repository/lifecycle
...
- [ ] 3.3 Send only synthetic fixture requests through preproduction, inspect metrics, traces,
...
- [ ] 4.1 Copy the authoritative compatibility table into the readiness record and attach one proof
...
- [ ] 4.2 Send the exact valid unversioned request shape from the committed previous-release Flutter
...
- [ ] 4.3 Diff `app/` and its generated client against the candidate base, regenerate OpenAPI and the
...
- [ ] 5.1 Run all server automated gates on the candidate: unit/integration coverage, server E2E,
...
- [ ] 5.2 Run all React Native automated gates: Orval drift, Expo type generation if required by CI,
...
- [ ] 5.4 Add `docs/react-native-migration/inbox/<date>-activity-release-device-passes.md`, tagged
...
- [ ] 6.1 In the readiness record, write the later rollout ticket's ordered procedure: deploy the
...
- [ ] 6.2 Write executable mobile release/OTA rollback where runtime compatibility permits and prior
...
- [ ] 7.1 Update the Activity entry in
...
- [ ] 7.2 Reconcile `docs/mobile/architecture-book/features.md` and any affected topical page with
...
- [ ] 7.3 Verify the committed CI proofs cover G3/G3a, telemetry privacy, legacy compatibility, and
