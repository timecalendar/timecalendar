...
- [ ] 1.4 **Applier:** move the file to `server/src/scripts/activity-capacity/production-aggregates.sql`
      when you create that directory (it currently sits in the change folder, where it was reviewable
      before being dispatched). Keep the contents byte-identical to what the Founding Engineer was
      given — if you need to change a query, re-post it.
- [ ] 1.5 When the results arrive on TIM-394, transcribe the buckets into the gate document (§7.1)
      and re-run §3/§5 at real scale, replacing every provisional-scale label.

## 2. Query shapes — the single source of truth

- [ ] 2.1 Add `server/src/scripts/activity-capacity/queries.ts` exporting the two parameterized SQL
...
- [ ] 2.2 Add a file-header comment recording design D1: Ticket 2 **relocates** this module into
...
- [ ] 3.1 Add `server/src/scripts/activity-capacity/fixtures.ts` with a deterministic seeder (fixed
...
- [ ] 3.2 Seed the **background corpus** (design D3) with server-side
...
- [ ] 3.3 Run `ANALYZE "calendar_log"; ANALYZE "calendar";` at the end of seeding — without fresh
      statistics every plan captured afterwards is meaningless.
- [ ] 3.4 Verify: seed at provisional scale locally, then confirm row counts and cohort shapes with a
      counting query. Record the wall-clock seed time in the gate document.

## 4. Redaction

- [ ] 4.1 Add `server/src/scripts/activity-capacity/redact.ts` — `redactPlan(text: string): string`
      replacing UUIDs and single-quoted string literals with `‹redacted›`. Everything the harness
      prints goes through it (design D5).
- [ ] 4.2 Add `redact.test.ts` covering: a UUID inside an `Index Cond`, a quoted token literal, a
...
- [ ] 5.1 Add `server/src/scripts/activity-capacity/cli.ts` with subcommands `seed`, `explain`,
...
- [ ] 5.2 `measure`: for every cohort × history variant, sample first-page and following-page
...
- [ ] 5.3 `measure`: report the serialized **v1** page byte distribution — map rows through the v1
...
- [ ] 5.4 `measure`: record `monitorEventLoopDelay` max and heap growth while running representative
      concurrent page reads, so the memory/event-loop gate has a number.
- [ ] 5.5 `explain`: run `EXPLAIN (ANALYZE, BUFFERS)` for the keyset and unread-count queries per
...
- [ ] 5.6 Add `activity:capacity:seed` and `activity:capacity:measure` scripts to
      `server/package.json`, following the existing `db:seed` / `app:tsnode` pattern.
- [ ] 5.7 Add `server/src/scripts/activity-capacity/README.md`: prerequisites, the exact commands,
...
- [ ] 6.1 Add `server/src/scripts/activity-capacity/plan.test.ts`: seed a bounded corpus through the
...
- [ ] 6.2 Record the corpus size and the test's measured runtime here: `corpus = ____ rows / ____
      calendars, runtime = ____ s`. Budget: **30 s**. If the corpus needed to make the planner behave
      exceeds it, shrink to the affordable corpus, downgrade the assertion to the invariants it can
      honestly make (keyset ordering, tie-breaker determinism, bounded row estimate), and write that
      downgrade into the gate document — do not weaken it silently.
- [ ] 6.3 Assert the ordering contract too: `(createdAt DESC, id DESC)` is stable across pages and
      deterministic when `createdAt` values tie.
      _(Verification: `npm test -- activity-capacity` in `server/`.)_

## 7. The gate document

- [ ] 7.1 Write `docs/react-native-migration/05-tech-specs/activity-capacity-gate.md` with: aggregate
      ...
      log.
- [ ] 7.2 The gate table starts from the specification's initial budgets — default 50-log page p95
...
- [ ] 7.3 State the **index verdict** explicitly: existing indexes sufficient, or a named composite
...
- [ ] 7.4 State the **default page size** verdict: the measured 50-log byte distribution and whether
      50 is safe. The contract shape and the 100 maximum do not change.
- [ ] 7.5 Record Ticket 2's standing obligation from design D1 (relocate `queries.ts`, keep the
      harness importing it) and Ticket 8's re-run instructions (exact commands, what to compare).
- [ ] 7.6 Cross-link: add a pointer to this document from `activity-revival.md`'s **Observability and
      capacity** section. Do not otherwise edit the specification.
- [ ] 7.7 **Redaction sweep.** Grep the full diff for UUIDs, `token`, quoted titles/locations, and
...
- [ ] 8.1 Green locally: `npm test -- activity-capacity redact` and `npx tsc --noEmit` in `server/`,
      plus `npm run lint` on the touched paths.
- [ ] 8.2 `openspec validate measure-activity-capacity-budgets --strict`.
- [ ] 8.3 Confirm the diff touches **no** `server/src/migrations/`, **no** `openapi/openapi.json`,
      **no** `mobile/`, and no production code under `server/src/modules/`.
- [ ] 8.4 Update the PR body's stage marker and hand off per `pipeline-core`.
- [ ] 8.5 If §1's aggregates are the only thing still outstanding at the end, move TIM-394 to
      `blocked` naming the Founding Engineer as unblock owner — and only then. Everything else must
