...
- [x] 1.1 In `server/src/modules/mailer/services/mailer.service.ts`, replace
...
- [x] 1.2 Guard `sendEmail`: when `SMTP_URL` is falsy, log one `warn` naming the missing
  configuration and return `undefined` without constructing a transport (design D2). Do not
  change the method signature.
- [x] 1.3 Call the transport accessor from **inside** `sendEmail`'s existing `try`, and
...
- [x] 1.4 Leave `server/src/config/constants.ts` untouched. Document in the mailer file
...
- [x] 2.1 Delete `process.env.SMTP_URL ??= "smtp://localhost:1025"` from
...
- [x] 2.2 Delete the `SMTP_URL: smtp://localhost:1025` entry and its two-line justification
  comment from the `environment:` block of `server/docker-compose.e2e.yml`. Change nothing
  else in that overlay.
- [x] 2.3 Leave `src/config/environments/development.ts`'s `SMTP_URL: "smtp://localhost:1025/"`
  in place — it points at a local Mailhog, not at a crash workaround (design D4).
- [x] 2.4 **Added during apply — a third shim the proposal did not find.** `ci/e2e-server.sh`
  `up_native()` boots the server with `SMTP_URL="smtp://localhost:1025"` inlined in its
  `exec env`. It is the same crash dodge as 2.1/2.2 (native mode is the Docker-less
  macOS-runner path for the same e2e stack), and the ADDED requirement "No environment sets
  a placeholder SMTP URL to survive boot" forbids it. Removed. Leaving it would have meant
  the native e2e path still masks the very defect this change fixes.

## 3. Add the regression test

- [x] 3.1 Add `server/src/modules/mailer/services/mailer.service.test.ts`. Mock
...
- [x] 3.2 Disabled-path cases, with `SMTP_URL` forced to `""`: compiling a testing module
...
- [x] 3.3 Configured-path cases, with `SMTP_URL` forced to a real-looking URL:
...
- [x] 5.1 No-SMTP full-app boot: from `server/`, with `SMTP_URL` guaranteed absent from the
...
- [x] 5.2 Empty-string boot: repeat 5.1 with `SMTP_URL=""` exported. Same result.
- [x] 5.3 `/health` half of the AC: run `./ci/e2e-server.sh up` (Docker) with the compose
  ...
  skip this criterion.
- [x] 5.4 Confirm the configured path still works end to end: repeat 5.1 with
...
- [x] 7.1 Architecture Book update: **N/A**, and say so in the PR body rather than leaving
  ...
  reverse.
- [x] 7.2 Update `docs/agent-dev-environment.md` only if a documented server command or CI
  gate changes shape. It does not today — record as N/A if so; do not add unrelated docs.

## 8. Scope and sensitive-surface audit

- [x] 8.1 Read the final diff. Confirm it touches only: the mailer service, its new test,
  `server/src/generate-openapi.ts`, `server/docker-compose.e2e.yml`, `ci/e2e-server.sh`
  (added by 2.4), and this OpenSpec change.
...
- [x] 8.2 Confirm no listed sensitive surface is touched, and that `server/src/app.module.ts`
...
- [x] 8.3 Run `openspec validate boot-server-without-smtp-url --strict` and record the exact
