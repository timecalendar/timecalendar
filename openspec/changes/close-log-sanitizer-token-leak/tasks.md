...
- [ ] 1.1 In `server/src/config/observability/sanitize-log.ts`, replace the id rule
...
- [ ] 1.2 In the same chain, add `["']?` before `\s*[:=]` in the sensitive-key rule **and**
...
- [ ] 2.1 In `server/src/config/observability/sanitize-log.test.ts`, add a table-driven case
...
- [ ] 2.2 Assert the two shapes that leak on `main` today — leading `-` and trailing `-` —
  ...
  passing vacuously.
- [ ] 2.3 Add a case asserting a JSON-quoted `"token"`, `"password"`, and `"secret"` key is
...
- [ ] 3.1 Add cases pinning the survival table in `design.md` Decision 4 to the values the
...
- [ ] 3.2 Add a case pinning the allow-listed structured keys `school`, `queue`, `action`,
  `service.name`, and `service.instance.id` as surviving with their values verbatim.
- [ ] 3.3 Pin `trace_id` as **`[id:redacted]`**, matching current behavior — a 32-hex value is
...
- [ ] 4.1 In `server/src/config/observability/telemetry-logger.test.ts`, add a test that
...
- [ ] 5.1 Run `cd server && npx jest src/config/observability/ --maxWorkers=2`; confirm all
...
- [ ] 5.2 Run `cd server && npm run build` and `npm run lint`; confirm tsc and ESLint are clean
