...
- [x] 1.1 Rewrite `ci/certificates/generate-certificates.sh` to resolve its own directory
...
- [x] 1.2 Add `ci/certificates/ensure-certificates.sh` — the idempotent guard. Regenerate by
...
- [x] 2.1 In `bin/server-compose.sh`, run `"$repo_root/ci/certificates/ensure-certificates.sh"`
...
- [x] 2.2 In `bin/setup-dev.sh`, run the guard before step 3, capturing its stdout. When it
...
- [x] 2.3 In `bin/setup-dev.sh` step 4, when the reachability check returns `000`, re-run the
...
- [x] 2.4 Update the `bin/setup-dev.sh` header comment so its numbered list matches the script's
  actual steps and points at `docs/agent-dev-environment.md` for the certificate mechanism
  rather than restating it.

## 3. Untrack the pair

- [x] 3.1 Add `ci/certificates/cert.pem` and `ci/certificates/key.pem` to the root `.gitignore`
...
- [x] 4.1 In `.github/workflows/ci-build-deploy.yml`, change the `test` job's "Start Postgres and
...
- [x] 4.2 Confirm no other CI or scripted path starts nginx or reads the pair:
...
- [x] 4.3 Review the sensitive-surface diff: `git diff -- .github/workflows/ ci/certificates/`.
...
- [x] 5.1 In `docs/agent-dev-environment.md` §4, add the mechanism next to the existing
...
- [x] 5.2 In `README.md`, add one sentence to the Docker section saying the dev TLS certificate
...
- [x] 5.3 Grep the **whole** of every document touched (not only the changed hunks) for
  ...
  - Result: `added: 0`, `preExisting: 3`, all three in `docs/agent-dev-environment.md` on
    §5 worktree-layout lines this change does not touch, all one pattern id
    (`host-home-directory`). This branch introduces nothing, which is the dated
    proceed-and-report shape; the pre-existing lines belong to the scrub ticket, not here.

- [x] 5.4 Confirm no Architecture Book change is required: `grep -rn "certificat" docs/mobile/`
  ...
  - Result: every hit is about **code signing** — EAS/Apple distribution certificates, the Play
    upload certificate, and the OTA update-signing certificate embedded at build time. None
    concerns the local nginx TLS proxy, so nothing under `docs/mobile/` is contradicted. No Book
    file and no changelog entry.
...
- [x] 6.1 From a clean state (`rm -f ci/certificates/*.pem`), run `bin/server-compose.sh up -d`.
  ...
  - **Deviation, pre-existing and unrelated to this change.** `bin/server-compose.sh up -d`
    provisioned the pair as designed, but the nginx container then restart-loops on this Linux
    host with `[emerg] host not found in upstream "host.docker.internal"` —
    `server/nginx.conf:17`. That alias is supplied automatically by Docker Desktop (macOS) and
    not by Docker Engine on Linux, which needs an `extra_hosts: host-gateway` entry that
    `server/docker-compose.yml` does not declare. Both files are byte-identical to `origin/main`
    (`git diff origin/main -- server/nginx.conf server/docker-compose.yml` is empty), so the
    fault predates this branch, and fixing it means editing the Compose file, which this
    change's non-goals exclude. Reported separately to the Founding Engineer.
  - What was proved instead, isolating only that one variable: the same `server/nginx.conf` and
    the same `ci/certificates` bind mount, in an `nginx` container given the host-gateway alias
    Linux needs, starts cleanly with **no certificate error** in its logs — and 6.2 below then
    verifies the chain through it.

- [x] 6.2 Prove the certificate actually verifies through nginx:
...
- [x] 6.3 Run `bash bin/setup-dev.sh` and confirm step 4 reports the API responded (any non-`000`
  status) rather than the failure message. Note in the PR body that step 3 skipped as
  not-macOS and that step 1 used sudo.
  - Ran against the proxy from 6.1's note (`TIMECALENDAR_TLS_PORT` pointed at it, since `:1443`
    on this host belongs to another checkout). Step 4: `✓ API responded (HTTP 502)`. Step 1
    wrote `/etc/hosts` via sudo; step 3 skipped as not-macOS; the `:3005` backend check fails
    because the API server is not running, which is outside this change.

- [x] 6.4 Assert the acceptance criteria directly and paste the output:
...
- [x] 6.5 Tear down: `bin/server-compose.sh down`. Confirm `git status --short` shows only the
  intended changes and no `.pem` is tracked or untracked-and-visible.

- [x] 6.6 Run `npx openspec validate provision-dev-tls-certificate --strict` and, before merge,
...
- [x] 6.7 Run `bash -n` (or `shellcheck` where available) over
...
- [x] 7.3 Run the `disclosure-scan` preflight once more on the finished branch with no text
  ...
  - Result: `added: 0`; neither `.pem` path appears in `findings` or in `scanned.files`. The
    deletion landed. The 3 pre-existing findings are the ones recorded under 5.3.
