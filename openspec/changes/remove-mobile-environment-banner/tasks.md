...
- [ ] 1.1 In `mobile/src/features/environment/ui/environment-runtime-gate.tsx`, delete
...
- [ ] 1.2 Drop every import the file no longer uses (`useTranslation` stays only if the recovery copy
      still needs it — it does; `SafeAreaView` stays for recovery). Verify no unused import or style
      survives; ESLint is the check.
- [ ] 1.3 Remove `NonProductionEnvironmentMarker` from
...
- [ ] 1.4 In `mobile/src/features/environment/ui/environment-runtime-gate.test.tsx`, delete the
...
- [ ] 1.5 Remove `"environment.marker"` from `mobile/src/i18n/locales/en.json` and
...
- [ ] 2.1 Add `"environment.selector.a11yLabel"` to both locales, in the `environment.selector.*`
      block so key order stays parallel: EN `"Environment, {{value}}"`, FR
      `"Environnement, {{value}}"`.
- [ ] 2.2 In `mobile/src/features/environment/ui/environment-settings-control.tsx`, compute the
...
- [ ] 2.3 In `environment-settings-control.test.tsx`, assert the row's `accessibilityLabel` includes
...
- [ ] 3.1 In `mobile/.maestro/environment-switch.yaml`, delete both
...
- [ ] 3.2 Keep the switch itself unchanged: tap `id: "settings-environment"`, tap `"Preproduction"`,
      assert `"Clear data and switch?"`, tap `"Clear and switch"`.
- [ ] 3.3 Replace the post-switch banner wait with: `extendedWaitUntil` on the `"Settings"` tab
...
- [ ] 3.4 Update the file's header comment to say what it now proves and why the selectors take the
...
- [ ] 4.1 In `docs/mobile/architecture-book/decisions/043-backend-environment-reset.md`, replace the
...
- [ ] 4.2 In `docs/mobile/architecture-book/features.md`, change the `environment` row's
...
- [ ] 4.3 Append a dated entry to `docs/mobile/architecture-book/CHANGELOG.md` recording the rule
...
- [ ] 4.4 In `docs/react-native-migration/inbox/2026-08-27-environment-switch-device-pass.md`, delete
...
- [ ] 4.5 Confirm this change introduces no new credential, device-install or console step, so no new
      `(HUMAN: …)` inbox note is required — the existing environment-switch note absorbs it.

## 5. Local-green verification

- [ ] 5.1 `cd mobile && npx tsc --noEmit`.
- [ ] 5.2 `cd mobile && npm run lint` — this carries the FR/EN i18n parity rule and the unused-import
      / unused-style checks that prove step 1 is complete.
- [ ] 5.3 `cd mobile && npm test -- --coverage` (the coverage gate is part of the suite). The
