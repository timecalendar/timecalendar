...
- [ ] 1.1 Give each of the three page-boundary `scrollUntilVisible` steps
...
- [ ] 1.2 Restore the `tie-higher` traversal's `timeout` to `60000`, so all three
  page-boundary scrolls carry the suite-standard bound.
- [ ] 1.3 Replace the two comment lines that justified the widened bound with the measured
...
- [ ] 1.4 Append the two relative-order assertions after the `older-anchor` step —
...
- [ ] 1.5 Verify the file still parses as a Maestro flow and that the flow remains
...
- [ ] 2.1 In `mobile/e2e/activity-maestro-selectors.test.ts`, delete the
  `"gives only the row-50 pagination traversal the measured wider bound"` case and its
  `120000` expectation.
- [ ] 2.2 Add a case that derives the page boundary from its two real sources: read the
...
- [ ] 2.3 Add a case pinning the traversal budget: the three page-boundary scrolls appear
  ...
  loosening it to `[\s\S]*`.
- [ ] 2.4 Add a case pinning both relative-order assertions, in the asserted direction.
- [ ] 2.5 **Prove the discrimination, do not assume it.** With the new test file in place,
...
- [ ] 3.1 In `docs/mobile/architecture-book/testing.md`, extend the Activity E2E bullet
...
- [ ] 3.2 Append a dated entry to `docs/mobile/architecture-book/CHANGELOG.md` naming the
  rule change (migration-approach §7). No ADR: this refines an existing E2E rule, it does
  not add a load-bearing decision.
- [ ] 3.3 Re-read `testing.md`'s existing Activity paragraph and confirm the "50-row client
...
- [ ] 4.1 `cd mobile && npx jest e2e/activity-maestro-selectors.test.ts e2e/maestro-selectors.test.ts --maxWorkers=4`
  — both green. (Use `--maxWorkers=4`; a bare pattern matches the worktree path here.)
- [ ] 4.2 `cd mobile && npx tsc --noEmit` and `npm run lint` — green.
- [ ] 4.3 `openspec validate repair-activity-e2e-page-boundary-traversal` and
  `git diff --check` — clean, with no stray debug artifact.
- [ ] 4.4 Confirm the diff touches only `mobile/.maestro/activity.yaml`,
