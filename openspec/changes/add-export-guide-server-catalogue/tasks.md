...
- [x] 8.5 After pushing the implementation head, confirm the existing server CI test/OpenAPI drift job and mobile generation/typecheck job execute on that exact SHA and are green. Treat missing contract drift or focused server proof as implementation rework; do not add a human merge gate or perform rollout.
