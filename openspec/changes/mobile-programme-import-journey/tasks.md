...
- [ ] 1.1 Add `intranetUrl: string | null` to `SchoolListItem` in
...
- [ ] 1.2 Create `mobile/src/features/onboarding/draft/types.ts`: `ImportInstitution`,
...
- [ ] 1.3 Create `draft/context.tsx`: `ImportDraftProvider` + `useImportDraft()` exposing
...
- [ ] 1.4 Add `useImportCreateFields()` (or an exported pure `toCreateFields(draft)`) deriving
...
- [ ] 1.5 Add `mobile/src/features/onboarding/draft/index.ts` and re-export the draft surface from
      `mobile/src/features/onboarding/index.ts`
- [ ] 1.6 Unit tests: the derivation table (listed → `schoolId` present and `schoolName` key
...
- [ ] 1.7 Verify: `npx jest src/features/onboarding src/features/school-selection --maxWorkers=4`

## 2. Routes and the provider mount

- [ ] 2.1 Wrap the nested `Stack` in `mobile/src/app/onboarding/_layout.tsx` with
      `ImportDraftProvider` (import through `@/features/onboarding` — routes must not reach seams
      directly, boundary B-3)
- [ ] 2.2 Add four thin routes, each a one-line re-export from `@/features/onboarding/ui`:
      `institution-name.tsx`, `programme.tsx`, `connect.tsx`, `import.tsx`. No colocated tests under
      `src/app/` (route-structure rule)
- [ ] 2.3 Verify each new deep link resolves: `timecalendar-dev://onboarding/{institution-name,
...
- [ ] 3.1 `features/onboarding/ui/institution-name-screen.tsx`: labelled `TextInput`, submit control,
...
- [ ] 3.2 Submit writes the `unlisted` draft, calls `clearSelection()` from
      `@/features/school-selection`, and pushes `/onboarding/programme`
- [ ] 3.3 Repoint `MissingSchoolAction` in `school-picker-screen.tsx` from `/onboarding/ical-url` to
      `/onboarding/institution-name`; update its `accessibilityHint` copy and the screen's test
- [ ] 3.4 Tests: empty and whitespace-only rejected (no draft, no navigation); 100 accepted / 101
...
- [ ] 4.1 `features/onboarding/ui/programme-screen.tsx`: `<Stack.Screen options={{ headerShown: true,
...
- [ ] 4.2 Field: label `onboarding.programme.fieldLabel`, placeholder `L3 Informatique` (prop only,
      never persisted), trimmed, 100-character normalized maximum with inline accessible validation
- [ ] 4.3 Continue (enabled only for a non-empty valid value) stores the normalized name and pushes
      `/onboarding/connect`; Skip stores `""` and pushes the same
- [ ] 4.4 Tests: Continue stores the trimmed value; Skip stores `""`; Continue unavailable on empty;
...
- [ ] 5.1 `features/onboarding/ui/connect-screen.tsx`: explanatory copy (Flutter
...
- [ ] 5.2 External-link action rendered only when `safeIntranetUrl(draft.institution.school.intranetUrl)`
...
- [ ] 5.3 Add a comment marking the Connect → manual-import edge as the future assistant insertion
      point, so a later project inserts there without touching the preceding screens
- [ ] 5.4 Tests: link shown for `https:` and `http:`; no link for null / empty / whitespace /
      `javascript:` / `file:` / unparseable / any unlisted draft; Back and Continue always present

## 6. Manual-import step (design D7)

- [ ] 6.1 `features/onboarding/ui/manual-import-screen.tsx`: explanatory copy (Flutter
...
- [ ] 6.2 Tests: both actions present and pushing the right routes; the screen contains no
      permission, validation, create, or retry logic

## 7. Creation and failure-report wiring (design D3, D7, D9)

- [ ] 7.1 `calendar-sources/data/create.ts`: export `CalendarImportFields`; `createCalendar(url,
...
- [ ] 7.2 `calendar-sources/data/user-calendars/add-calendar.ts`: `addCalendarFromUrl(url, fields)`
      forwards the fields; the create → resolve → upsert chain and its pending/error semantics are
      otherwise unchanged
- [ ] 7.3 `qr-scan-screen.tsx`: read the derived fields from `@/features/onboarding`, pass them to
...
- [ ] 7.4 `ical-url-screen.tsx`: same create wiring and same success/failure draft handling; **remove**
...
- [ ] 7.5 `feedback`: accept a `calendarName` route parameter in `feedback-screen.tsx`, add it to
      `FeedbackContext` and `buildFeedbackDto` in `data/use-send-feedback.ts` (trimmed, omitted when
      empty). `gradeName` stays unsent
- [ ] 7.6 Tests (`data/`, mocking `src/api/mutator` per `testing.md`): listed create body has
...
- [ ] 7.7 Tests (`ui/`): a failed import leaves the draft intact and the URL retryable; the report
...
- [ ] 8.1 `calendar-sources/data/effective-name.ts`: pure `effectiveCalendarName(stored): string | null`
      (trim, empty → `null`); export from the `data/` sub-barrel and the feature barrel
- [ ] 8.2 `user-calendars-screen.tsx`: replace `calendar.name || t("userCalendars.namePlaceholder")`
...
- [ ] 8.3 Replace the `userCalendars.namePlaceholder` key with `userCalendars.nameFallback`
      ("My timetable" / "Mon emploi du temps") in both catalogs
- [ ] 8.4 Tests: empty, whitespace-only, padded, normal, and >100-character stored names; the stored
      value is never mutated

## 9. i18n, Architecture Book, and the human-only note

- [ ] 9.1 Add every new key to `mobile/src/i18n/locales/en.json` **and** `fr.json` under the
...
- [ ] 9.2 `docs/mobile/architecture-book/navigation.md`: update the onboarding-group paragraph with
...
- [ ] 9.3 `docs/mobile/architecture-book/features.md`: update the `onboarding` and `calendar-sources`
      rows, and the cross-feature contracts bullets for the draft handoff, the effective display
      name, and the feedback context
- [ ] 9.4 Write `docs/mobile/architecture-book/decisions/045-ephemeral-calendar-import-draft.md`
...
- [ ] 9.5 Append a dated entry to `docs/mobile/architecture-book/CHANGELOG.md`
- [ ] 9.6 Add `docs/react-native-migration/inbox/2026-08-30-import-journey-device-pass.md` tagged
...
- [ ] 10.1 **Early:** `openspec validate mobile-programme-import-journey --strict`, then a dry-run of
      `openspec archive` to catch a MODIFIED/ADDED header error before the end of the change
- [ ] 10.2 Extend `mobile/.maestro/onboarding.yaml` only where cheap and stable: after the school-step
...
- [ ] 10.3 `cd mobile && npx tsc --noEmit && npm run lint && npm test -- --coverage` — all green,
      90% logic / 70% global thresholds met
- [ ] 10.4 `git diff --stat origin/main -- app/` is empty (no Flutter file changed) and
      `git diff --stat origin/main -- mobile/src/api/generated openapi/` is empty (contract untouched)
- [ ] 10.5 Update the PR body's stage line and its Affected/Sensitive sections if the shape of the
