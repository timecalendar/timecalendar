...
- [ ] 1.1 Replace the nullable onboarding draft state with the discriminated Stack-scoped journey
...
- [ ] 1.2 Add pure listed/unlisted Programme and Connect gate resolution, including empty programme
...
- [ ] 1.3 Add the pure earliest-legal-route and guarded-handoff model for guide/manual/QR/iCal entry,
...
- [ ] 2.1 Implement the export-guide journey coordinator over the T2 repository/resolver with
...
- [ ] 2.2 Build the native unlisted provider selector from every ordered T2 selectable provider,
...
- [ ] 3.1 Build the guide-page screen using pinned plain-text content, localized progress, readable
...
- [ ] 3.2 Add validated `expo-image` presentation with correct alt/caption semantics and a stable
...
- [ ] 3.3 Add thin provider-selection and dynamic guide-page route exports and explicitly register
...
- [ ] 3.4 Wire Next-only contiguous `router.push` transitions, final completion/manual handoff, and
...
- [ ] 3.5 Apply the shared guard to manual import, QR scan, and iCal URL before any create action;
...
- [ ] 4.1 Add typed FR/EN shell keys for selector, route titles, progress, loading, every blocking
...
- [ ] 4.2 Build the blocking loading/error surface with ordinary Back, screen-reader live status,
...
- [ ] 4.3 Add closed typed builders for all nine export-guide Analytics events and sanitized
...
- [ ] 5.1 Update ADR 047 to make completed-guide route legality current, update `navigation.md` and
