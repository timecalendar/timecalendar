...
- [ ] 1.1 Extend `mobile/e2e/test_run_e2e.sh` (or add one equivalently focused shell proof
...
- [ ] 1.2 In `mobile/.maestro/feedback.yaml`, insert the bounded downward reveal immediately
...
- [ ] 2.1 Run `cd mobile && npx prettier --check .maestro/feedback.yaml` and inspect the
      parsed/indented command block to confirm the selector, direction, timeout, and command
      nesting are valid Maestro YAML.
- [ ] 2.2 Run `bash -n mobile/e2e/run_e2e.sh mobile/e2e/test_run_e2e.sh` and
...
- [ ] 3.1 Re-read `docs/mobile/architecture-book/testing.md`, `navigation.md`, ADR 038, and
...
- [ ] 3.2 Audit the final diff and confirm it contains only this OpenSpec change, the Feedback
...
- [ ] 4.1 Run `openspec validate reveal-feedback-row-in-e2e` and confirm every modified
