...
- [ ] 1.1 Re-read the latest [TIM-360](/TIM/issues/TIM-360) evidence before editing and
...
- [ ] 2.1 In `k8s/timecalendar/templates/server-deployment.yaml`, change only
...
- [ ] 2.2 Render `templates/server-deployment.yaml` with Helm and manually inspect the
...
- [ ] 3.1 Extend `ci/test-timecalendar-chart.sh` with a bounded assertion that renders
  ...
  assertion.
- [ ] 3.2 Keep `.github/workflows/ci-build-deploy.yml` unchanged because its existing
...
- [ ] 4.2 Compare `docs/agent-dev-environment.md` health-route and chart-CI guidance with
...
- [ ] 5.1 Run `helm lint k8s/timecalendar` and record the successful result.
- [ ] 5.2 Run `./ci/test-timecalendar-chart.sh` and record the successful probe-split
...
- [ ] 5.3 Run `openspec validate split-k8s-liveness-from-readiness` and record the
  successful result.
...
- [ ] 6.1 Inspect the implementation diff and confirm it changes only
