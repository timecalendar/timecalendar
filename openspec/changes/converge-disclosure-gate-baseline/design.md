...
- Commit one generated, string-free baseline file with separate named lanes that both mechanisms
  consult under shared file and verdict rules.
...
## Decision 1 — Commit two generated content lanes in one backward-compatible file

`ci/disclosure-baseline.json` has version 1, an `entries` array for the preflight, and a
`ciEntries` array for CI. Every item in either array is exactly
`{"path": …, "id": …, "count": …}`: a repository-relative path, a safe label, and a positive
integer. Each lane independently rejects duplicate `(path, id)` keys. This preserves the deployed
preflight loader: it validates `version` and `entries` and ignores sibling top-level fields.

`entries` remains the accepted preflight census: 45 paths / 50 entries / 100 occurrences at the
reference measurement. `ciEntries` is generated by `ci/disclosure-scan.mjs` from the same tracked
tree and exclusion, using CI's own safe finding classes. At the current merge base, CI's always-on
sources measure 55 paths / 56 entries / 134 occurrences: `derived-identity` 47/111,
`home-directory-path` 6/12, and `email-address` 3/11 (entries/occurrences). Path-source matches enter
neither content lane.

The complete JSON file records *how much* each detector finds, never *what* it matched. It MUST scan
clean as a whole. That is why this one artifact is safe to commit while the pattern list is not.
...
## Decision 3 — Share file and verdict rules, with separate named generated lanes
...
The seven committed preflight ids and the six CI finding classes have zero shared labels. Combining
them into one per-path budget is not faithful either: at the current tree, 9 of the 45
preflight-baselined paths would exceed that aggregate budget by 20 CI occurrences. The file
therefore keeps the vocabularies explicit as `entries` and `ciEntries`.

What converges is the behavioral contract:
...
- **the baseline file**, consulted by both through separate named generated lanes. The preflight
  reads only `entries`; CI layer A reads only `ciEntries`. A path may legitimately be pinned in both
  lanes because each comparison is independent.
...
## Decision 6 — Mechanically remeasure only the lane CI can reproduce

Add one CI step that re-measures `ciEntries` at the commit under test and fails when any committed
CI entry **exceeds** the measured count. This catches a pin left behind after a scrub and a pin
quietly raised to cover a finding. Under-pinning needs no stale-pin check because the scan itself
already fails on it.

CI cannot mechanically remeasure `entries` without the out-of-repository preflight patterns, so it
MUST NOT claim to validate that lane. When that input is present, the preflight enforces `entries`
itself.

CI baselines a configured source only when it is present during generation. If it is absent,
generation records degraded coverage and remeasures the always-on classes. If it later becomes
present, an unpinned `configured-pattern` finding fails closed until a deliberate regenerated
`ciEntries` lane is reviewed.
...
- **The preflight lane legitimises 100 occurrences, and the CI lane pins its independently measured
  footprint.** That is the point and the cost: it converts an
