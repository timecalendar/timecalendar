...
- [ ] 1.1 Add `extra_hosts: ["host.docker.internal:host-gateway"]` to the `nginx` service in
...
- [ ] 1.2 Confirm the E2E overlay inherits it without gaining anything else:
...
- [ ] 2.1 Extend `assertScopedModel` in `bin/verify-server-compose.mjs` with the host-gateway
...
- [ ] 2.2 Keep the harness `config`-only. No `up`, `down`, `stop`, `rm`, prune, or any other
...
- [ ] 2.3 Prove the assertion actually bites: with the mapping temporarily removed from
...
- [ ] 3.1 In `bin/setup-dev.sh` step 4/4, on the `code = 000` branch only, query the nginx
...
- [ ] 3.2 Guard the query for `set -euo pipefail`: capture with `2>/dev/null || true` so a
...
- [ ] 3.3 Do not touch the `--http-status` or `--compose-config` argument modes.
...
- [ ] 4.1 `bash -n bin/setup-dev.sh` (and any other changed shell script).
- [ ] 4.2 `node bin/verify-server-compose.mjs` green. It needs the main checkout **plus one
...
- [ ] 4.3 Live proof on this Linux host: `bin/server-compose.sh up -d` then
...
- [ ] 4.4 Exercise the step 4/4 branch you changed: run `bin/setup-dev.sh` (or just its step 4/4
...
- [ ] 4.5 `openspec validate resolve-dev-nginx-host-gateway --strict`.

## 5. Scope audit

- [ ] 5.1 `git diff --check` and review the final path diff. The only changed paths outside
...
- [ ] 5.2 No Architecture Book update is due: nothing under `mobile/` changes, and the local
...
- [ ] 5.3 Record the macOS assumption in the PR body per `design.md` Decision 4 — the reasoning,
  and that it is explicitly **not** a merge gate. No stage may convert it into one.
- [ ] 5.4 State in the PR body that `bin/verify-server-compose.mjs` is a manual-only harness
