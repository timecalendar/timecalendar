...
- [ ] 1.1 In `mobile/.maestro/activity.yaml`, add a required
...
- [ ] 1.2 Keep the existing required tie-higher, tie-lower, and older-anchor scroll/assert pairs in
...
- [ ] 2.1 Extend `mobile/e2e/activity-maestro-selectors.test.ts` with a focused source-contract test
...
- [ ] 2.2 Update the Activity fixture section in `mobile/e2e/README.md` to explain that the stable
...
- [ ] 2.3 Architecture Book/ADR assessment: record this task as N/A because the change is a leaf
...
- [ ] 3.2 Run `cd mobile && npm run lint` and confirm the shared flow support/test edits are clean.
- [ ] 3.3 Run `openspec validate repair-ios-activity-tie-row-reachability` and confirm every proposal,
  design, delta-spec, and task artifact is valid.
- [ ] 3.4 Inspect the final diff and confirm no sensitive surface, generated native output, app
