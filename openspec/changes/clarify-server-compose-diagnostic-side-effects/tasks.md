...
- [ ] 1.2 Keep the pure-mode exception exact: the selected project-name diagnostic and setup
...
- [ ] 2.1 Update `docs/agent-dev-environment.md` §4 so its troubleshooting commands use the same
...
- [ ] 2.2 Preserve the first-use guard, renewal window, lifetime override, explicit-rotation, nginx
...
- [ ] 3.1 Extend `bin/verify-server-compose.mjs` with focused static assertions over
...
- [ ] 3.2 Assert the wrapper's established order remains `project-name` early exit → certificate
  ...
  verb.
- [ ] 3.3 Prove the assertions bite by saving the relevant contract file to run-owned scratch,
...
- [ ] 4.1 Run `node bin/verify-server-compose.mjs` and record its pass. Before and after, capture
...
- [ ] 4.2 Run `git check-ignore -v ci/certificates/cert.pem ci/certificates/key.pem` and
...
- [ ] 4.3 Run `openspec validate clarify-server-compose-diagnostic-side-effects --strict` and
  `git diff --check`.

## 5. Architecture Book, scope, and sensitive-surface audit

- [ ] 5.1 Re-read `docs/mobile/architecture-book/architecture.md` and `testing.md`. Record the
...
- [ ] 5.2 Review the final path diff. Outside `openspec/`, only
...
- [ ] 5.3 Run the repository disclosure preflight over the whole branch and the exact PR text. Any
...
- [ ] 6.1 Rehearse `openspec archive clarify-server-compose-diagnostic-side-effects -y` in a fresh
