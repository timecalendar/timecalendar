...
- [ ] 1.1 Before editing anything, materialise the `origin/main` chart and the real
...
- [ ] 2.1 In `k8s/timecalendar/templates/server-deployment.yaml`, add the annotations block
...
- [ ] 2.2 In `k8s/timecalendar/values.yaml`, add `podAnnotations: {}` under `server:`
...
- [ ] 2.3 Do not touch `templates/web-deployment.yaml`, any ConfigMap key, any secret, or
  any other chart file.

## 3. Extend the committed CI proof

- [ ] 3.1 In `ci/test-timecalendar-chart.sh`, add assertions alongside the existing
...
- [ ] 3.2 Confirm no change is needed to `.github/workflows/ci-build-deploy.yml` — the
...
- [ ] 4.1 Render the implementation against the same two environment values files and
  ...
  re-runs.
- [ ] 4.2 Confirm the bare chart is also unchanged:
  `diff <(helm template timecalendar "$S/k8s/timecalendar") <(helm template timecalendar k8s/timecalendar)`
  must be empty.
- [ ] 4.3 Prove the stamped render changes exactly the intended two lines and nothing else:
...
- [ ] 4.4 Confirm the web Deployment is byte-identical in the stamped render:
...
- [ ] 5.1 `helm lint k8s/timecalendar` passes.
- [ ] 5.2 `./ci/test-timecalendar-chart.sh` passes locally, including the new assertions.
- [ ] 5.3 `openspec validate add-chart-server-pod-annotations --strict` passes. (Run it
  before handoff — the archive step is the only thing that validates delta headers, and it
  runs behind the merge gate.)

## 6. Documentation

- [ ] 6.1 Mobile Architecture Book and decision log: record as **N/A** in the PR body and
...
- [ ] 6.2 `docs/agent-dev-environment.md`: update only if the chart CI invocation changes.
  Task 3.2 expects it does not — record as N/A if so.

## 7. Scope and sensitive-surface audit

- [ ] 7.1 Read the final `git diff --stat`. The only files may be
...
- [ ] 7.2 Confirm the diff contains no annotation *value*, no checksum annotation, no
  ...
  Flutter change.
- [ ] 7.3 Flag `k8s/` as a sensitive surface in the PR body and in every downstream handoff,
  and state explicitly that merging is not a deploy act because the default render is
  unchanged (evidence: task 4.1 digests).

## 8. QA

- [ ] 8.1 `QA: none`. No human-visible surface; chart template only. The render digests are
  the evidence.
