...
- [ ] 1.1 Add an end-to-end test in `ci/disclosure-scan.test.mjs` that creates a temporary base and
...
- [ ] 1.2 Extend the repository fixture across author name, author email, committer name, and
...
- [ ] 1.3 Add the two scope/control directions to the repository suite: a matching header only on
  ...
  remain distinct.
- [ ] 1.4 Add equivalent focused regression cases to the canonical contributor-preflight test
...
- [ ] 2.1 Refactor the existing merge-base-to-head commit-log collection in
...
- [ ] 2.2 Emit one baseline-free `commit-header` record per non-empty identity field with a location
...
- [ ] 2.3 Update scanner summaries/types/comments only where needed to include commit headers;
  confirm the lane never reads `ci/disclosure-baseline.json`, never uses `creditPaths`, and never
  prints a raw header value.
- [ ] 2.4 Run the focused header tests after implementation and confirm all four matching-field
...
- [ ] 3.1 In the canonical contributor-preflight implementation, extend its existing branch log
  record to carry the same four identity fields alongside hash and body, using the already resolved
  merge-base-to-head range.
- [ ] 3.2 Route each non-empty header field through the existing unnarrowed text matcher as
...
- [ ] 3.3 Run the canonical preflight tests and a black-box `disclosure-scan` request against a
...
- [ ] 4.1 Update `docs/agent-dev-environment.md` so the current disclosure-gate contract lists
...
- [ ] 4.2 Complete the Architecture Book assessment: this changes repository and CI tooling rather
...
- [ ] 4.3 Re-check the sensitive surface after implementation. Keep
