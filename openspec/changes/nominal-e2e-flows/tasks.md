...
- [x] 1.1 Add a `Calendar` fixture seeding one calendar with a known constant
...
- [x] 1.2 Add a `CalendarContent` fixture for that calendar with 2–3 events in
...
- [x] 1.3 Verify: from `server/`, `NODE_ENV=test PORT=3005 npm run db:init`
...
- [x] 2.1 Extend `app/lib/modules/shared/test_utils/test_utils.dart` with
...
- [x] 2.2 Verify: from `app/`, `flutter analyze` is clean for the changed file.

## 3. Onboarding + add-school flow

- [x] 3.1 Add `app/integration_test/onboarding_flow_test.dart`. With an empty
...
- [x] 4.1 Add `app/integration_test/calendar_flow_test.dart`. The test (one
...
- [x] 5.1 Update `app/integration_test/run_e2e.sh` to run **each**
...
- [x] 5.2 Delete `app/integration_test/app_test.dart` (superseded by the flow
  files; its happy path is folded into `onboarding_flow_test.dart`).
- [x] 5.3 Verify the non-device path locally: run `run_e2e.sh` on the dev host
...
- [x] 6.1 Update the `test-e2e` job in `.github/workflows/build.yaml` so it runs
...
- [x] 6.2 Verify the workflow YAML is well-formed (`actionlint`/`yq` if
  available, else a careful diff against the existing jobs). Triggering the run
  needs the repo owner — note this in the handoff.

## 7. Documentation

- [x] 7.1 Update `app/integration_test/README.md`: replace the "one test by
...
- [x] 8.1 From `app/`, `flutter analyze integration_test` and
  `flutter analyze lib/modules/shared/test_utils` are clean.
- [x] 8.2 Local non-emulator verification per task 5.3 done; record that the
