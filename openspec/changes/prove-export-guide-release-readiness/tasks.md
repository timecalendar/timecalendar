...
- [x] 1.1 Add a `NODE_ENV=test`-only export-guide seed for the enabled test flag and stable listed
...
- [x] 1.2 Add the immutable bilingual T4 fixture identity over all four initial providers and one
...
- [x] 1.3 Extend the real E2E server lifecycle checks to prove the seeded flag, school responses,
...
- [x] 2.1 Add a closed `--suite smoke|export-guide` argument to `mobile/e2e/run_e2e.sh`, preserving
...
- [x] 2.2 Add platform-neutral export-guide flows and nested helpers for listed exact mapping,
      ...
      before completion.
- [x] 2.3 Extend `mobile/e2e/maestro-selectors.test.ts` to recursively validate the dedicated suite's
...
- [x] 3.1 Add one shared cold-state production-guard Maestro flow for direct manual, QR, and iCal
...
- [x] 3.2 Add a bounded test/probe proving a production runtime rejects
...
- [x] 3.3 Add log assertions around the production guard proof so it cannot contact a live backend;
...
- [x] 4.1 Extend `.github/workflows/ci-mobile-e2e.yml` with a closed manual suite input: schedules
...
- [x] 4.2 Generate JSON and Markdown evidence summaries from runtime outputs for target SHA, run,
...
- [x] 4.3 Extend `mobile/e2e/test_ci_mobile_e2e.sh` with mutation-backed checks for exact-SHA reuse,
...
- [x] 4.4 Add a focused evidence checker which rejects different-SHA, missing, skipped, cancelled,
...
- [x] 5.1 Add a `(HUMAN: ...)` migration inbox record with the named iPhone/VoiceOver,
...
- [x] 5.2 Update `mobile/e2e/README.md`, `docs/mobile/architecture-book/testing.md`, and the
...
- [x] 5.3 Audit the final diff and documentation for scope: explicitly verify no OpenAPI/generated
...
- [x] 6.1 Run formatting and focused server seed/catalogue/lifecycle tests, mobile selector/harness/
...
- [x] 6.2 Run `openspec validate prove-export-guide-release-readiness --strict`, run the repository
...
- [x] 6.4 Record the physical matrix's actual results when an authorized device run is available;
