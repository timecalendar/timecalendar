...
- [ ] 1.1 Add `updateName(id, name)` to `data/user-calendars/repository.ts` — a single-column
      `db.update(userCalendars).set({ name }).where(eq(userCalendars.id, id))`, mirroring
      `setVisible`. **Not** an `upsert`.
- [ ] 1.2 Add `data/calendar-sources/effective-name.ts` exporting the pure
      `effectiveCalendarName(stored, fallback)` = `stored.trim() || fallback`; export from the
      feature barrel.
- [ ] 1.3 Tests: `repository.test.ts` asserts the query shape sets only `name` and filters by `id`,
...
- [ ] 2.1 Add `data/user-calendars/rename.ts` exporting `useRenameCalendar()` in the shape of
...
- [ ] 2.2 Split the failure domains: a rejected request → `isError` only, NOT `recordError`
...
- [ ] 2.3 Export from the `data/user-calendars` and `data` barrels; check for a barrel cycle the way
      `add-calendar.ts:7` documents (import siblings by full `@/` path, never the sub-barrel).
- [ ] 2.4 Tests (`rename.test.ts`, mocked at the mutator seam per `testing.md`): success persists the
...
- [ ] 3.1 Add `ui/rename-calendar-dialog.tsx`: RN `Modal` (`transparent`, `animationType="fade"`,
...
- [ ] 3.2 States `idle | pending | error`: Save disabled while pending or invalid; the entered text
...
- [ ] 3.3 Validation: block when `trim(value).length > 100` with inline text announced to screen
...
- [ ] 3.4 `testID`s `user-calendar-rename-dialog` / `-input` / `-save` / `-cancel`; input
...
- [ ] 3.5 Tests (`rename-calendar-dialog.test.tsx`): 101 chars blocked and no request; exactly 100
...
- [ ] 4.1 In `ui/user-calendars-screen.tsx`, replace the `Platform.OS === "ios" ? MenuView : trash
...
- [ ] 4.2 Android needs the `calendar-view-menu.tsx` `CalendarAndroidViewMenu` idiom verbatim —
...
- [ ] 4.3 Wire `rename` → open the dialog for that row; `delete` → the existing `confirmDelete`
      (`Alert`, destructive confirm, announce gated on the resolved write) unchanged.
- [ ] 4.4 Replace `calendar.name || t("userCalendars.namePlaceholder")` with
...
- [ ] 4.5 Tests in `user-calendars-screen.test.tsx`: both `Platform.OS` shapes render one overflow
...
- [ ] 5.1 In `features/calendar/data/sync/sync.ts`, after the existing `replaceAll(rows)` try/catch
...
- [ ] 5.2 **Do not** `upsert`, and do not route through `fromCalendarForPublic` — it hard-codes
...
- [ ] 5.3 On failure: keep the committed events, keep the last-good names, call
      `recordUnknownError(error, "calendar/sync-names")` (a context distinct from `"calendar/sync"`),
      set `isError`, and do not throw (design D3).
- [ ] 5.4 Tests in `sync.test.tsx`: a changed name calls `updateName`; a hidden calendar keeps
...
- [ ] 6.1 **Change the value** of `userCalendars.namePlaceholder` to `My timetable` / `Mon emploi du
      temps` (design D6 — a value edit, not a new key; TIM-391 is in flight on these same files).
- [ ] 6.2 Add flat keys to both `en.json` and `fr.json`: `userCalendars.rename.action`,
      `.title`, `.label`, `.save`, `.tooLong`, `.error`, `.retry`, `.renamed`.
- [ ] 6.3 Verify FR/EN parity and the typed-key surface (`i18next.d.ts`) — the existing i18n parity
...
- [ ] 7.1 In `server/src/scripts/seed-e2e-calendar.ts`, export
...
- [ ] 7.2 Add `mobile/.maestro/rename-seed.yaml` mirroring `import-seed.yaml` for that token
      (leading `launchApp: clearState: true`, `stopApp`, `openLink`, the optional iOS "Open" tap,
      `extendedWaitUntil timeout: 60000`).
- [ ] 7.3 Add `mobile/.maestro/user-calendar-rename.yaml` implementing the six steps in
...
- [ ] 7.4 Update `mobile/e2e/README.md`: the new flow, and the re-run caveat (the baseline assertion
      needs a freshly seeded server; CI re-seeds every run).
- [ ] 7.5 Server-side seed check stays green.
...
- [ ] 8.1 Update `docs/mobile/architecture-book/features.md` — the user-calendars entry gains the
...
- [ ] 8.2 Append a dated entry to `docs/mobile/architecture-book/architecture-changelog.md`.
- [ ] 8.3 Add a `docs/react-native-migration/inbox/` note tagged `(HUMAN: …)` for the device-only
...
- [ ] 9.1 `npx tsc --noEmit` and `npm run lint` green in `mobile/`.
- [ ] 9.2 `npm test -- --coverage --maxWorkers=4` in `mobile/` — suite green and the coverage gate
...
- [ ] 9.3 **Dry-run the archive early, not at the end**: `npx openspec validate mobile-calendar-rename`
...
- [ ] 9.4 Confirm `git diff --name-only origin/main...HEAD` touches nothing under `app/`, nothing
