...
- [x] 1.1 Add the QR-import attempt and discriminated state types under
...
- [x] 1.2 Implement `useQrImportController` as the sole owner of barcode, import, retry, Scan
...
- [x] 1.3 Keep synchronous controller-private guards for scan claiming, in-flight exclusion,
...
- [x] 1.4 Make invalid payload return to an armed scanning phase without recording; make failure
...
- [x] 2.1 Reduce `qr-scan-screen.tsx` to a composition shell that reads camera permission,
...
- [x] 2.2 Extract cohesive permission loading/request/settings views. Preserve all existing
...
- [x] 2.3 Extract the QR-only scanner plus invalid-payload and importing/progress presentation.
...
- [x] 2.4 Extract the terminal success and failure/recovery presentation. Success remains an
...
- [x] 2.5 Inspect component and file sizes after extraction. No component may be materially above
...
- [x] 3.1 Add direct controller-hook tests using deferred promises for a valid normalized scan and
...
- [x] 3.2 Prove invalid payload displays recoverable guidance, starts no operation, records nothing,
  and accepts the next valid scan without an explicit reset.
- [x] 3.3 Prove an initial backend rejection records once and retains recovery; a rejected retry
  records once per invocation; and rapid Retry plus camera inputs never start a concurrent or extra
  invocation.
- [x] 3.4 Change the current import fields and next camera payload after failure, then prove Retry
...
- [x] 3.5 Unmount with an invocation pending and settle it once by resolve and once by reject.
  Assert no navigation, draft clear, error record, React state update, or console warning in either
  case.
- [x] 3.6 Retain screen integration coverage for permission loading/request/settings/granted,
...
- [x] 3.7 Run the focused CI proof from `mobile/`:
...
- [x] 4.1 Run React Doctor from `mobile/` against every changed QR-import `.ts`/`.tsx` file with
...
- [x] 4.2 Run `npx tsc --noEmit` and `npm run lint` from `mobile/`; both must be green with the
  existing feature dependency direction and public barrels intact.
- [x] 4.3 Run the full mobile suite from `mobile/` with `npm test -- --runInBand` when practical.
...
- [x] 4.4 Run `openspec validate extract-mobile-qr-import-state-machine --strict` and
...
- [x] 5.1 Re-read `docs/mobile/architecture-book/features.md`, `navigation.md`, `testing.md`, and
...
- [x] 5.2 Walk `docs/mobile/architecture-book/definition-of-done.md`. Record automated axes as
...
- [x] 5.3 Audit the final path diff: outside this OpenSpec change, production/test edits are limited
...
- [x] 5.4 Push the implementation head and confirm the standard mobile CI run is for the exact PR
