...
- [ ] 1.1 Add `server/src/modules/calendar-log/models/dto/search-calendar-logs-v1.dto.ts` —
  ...
  - `limit?`: `@IsOptional() @IsInt() @Min(1) @Max(100)`, class default `= 50`. Do **not** add
...
- [ ] 1.2 Add `calendar-log-v1.dto.ts` — `CalendarLogV1 extends OmitType(CalendarLogGet, ["calendarToken"] as const)`
...
- [ ] 1.3 Add `@IsArray()` to the existing `GetCalendarLogsDto.tokens`. Change nothing else in that
  file.
...
- [ ] 2.1 Add `server/src/modules/calendar-log/models/calendar-log-cursor.ts`: `encodeCursor` /
  `decodeCursor` over base64url JSON `{ v: 1, a, c, i }`, where `a` and `c` are Postgres
  `timestamp` **text** renderings and `i` is a uuid (design D3).
- [ ] 2.2 `decodeCursor` throws `BadRequestException` with a constant message — never echoing the
...
- [ ] 2.3 Privacy-negative test: encode a cursor from a fixture row and assert the decoded payload
  contains no calendar token and no event title, location, or UID.

## 3. Repository queries

- [ ] 3.1 Add `CalendarLogRepository.getSnapshotTime(): Promise<{ asOf: Date; asOfText: string }>`
...
- [ ] 3.2 Add `CalendarLogRepository.searchPage({ tokens, asOfText, cursor, limit })` using the
...
- [ ] 3.3 Add `CalendarLogRepository.countSince({ tokens, unreadSince, asOfText })` mirroring 3.2's
...
- [ ] 3.4 Repository integration tests (`calendar-log.repository.test.ts`):
...
- [ ] 4.1 Add `CalendarLogService.searchV1(payload)`:
...
- [ ] 4.2 Add `CalendarLogMapper.toCalendarLogV1(entity)` reusing the existing private
...
- [ ] 4.3 Add `server/src/modules/calendar-log/controllers/calendar-log-v1.controller.ts` —
...
- [ ] 4.4 Register the controller and any new providers in `CalendarLogModule`.
  - _Verify:_ `cd server && npx tsc --noEmit && npm run lint`.

## 5. Telemetry

- [ ] 5.1 Add `CalendarLogMetricsService` following `contact-metrics.service.ts`, with the three
  instruments and closed-union label types in design D10. Wire it into the v1 service path only.
- [ ] 5.2 Privacy-negative test: drive first-page, following-page, and invalid-cursor requests
...
- [ ] 6.1 v1 controller tests (`calendar-log-v1.controller.test.ts`) covering **every** row of the
...
- [ ] 6.2 Response-shape test: the serialized v1 item has no `calendarToken` key and the calendar's
  token string appears nowhere in the response body.
- [ ] 6.3 Unread-count tests: first page with `unreadSince` returns the exact count scoped to the
...
- [ ] 6.4 Mid-pagination insert test at the controller level: page 1, insert a new log, page 2 —
  assert no duplicate and no displaced row, and that the new log appears in neither page.
- [ ] 6.5 Privacy-negative log test: spy on `Logger` (and the `TelemetryLogger` path) across a
...
- [ ] 6.6 Legacy compatibility test: an existing valid array request returns the unchanged
  `CalendarLogGet[]` shape including `calendarToken`.
  - _Verify:_ `cd server && npx jest calendar-log`.

## 7. Contract, evidence, and green

- [ ] 7.1 Regenerate and commit the OpenAPI contract:
  ```bash
...
- [ ] 7.2 Regenerate and commit the mobile client (design D8):
  ```bash
...
- [ ] 7.3 Capture query-plan evidence (design D7). Seed representative fixtures for **1, 10, and
  ...
  table.
- [ ] 7.4 Add an index migration **only if** 7.3's plans miss the budget. If added, the justifying
...
- [ ] 7.5 Record the timezone invariant (design's risk list): confirm the DB session and Node
...
- [ ] 7.6 Architecture Book: **no change required.** `docs/mobile/architecture-book/data.md`
...
- [ ] 7.7 Local green: `cd server && npx tsc --noEmit && npm run lint && npm test`, and
  `cd mobile && npx tsc --noEmit`.
- [ ] 7.8 Measure against the provisional budgets (default 50-log page p95 < 250 ms; maximum
...
- [ ] 7.9 `openspec validate add-v1-calendar-log-search --strict` passes.
