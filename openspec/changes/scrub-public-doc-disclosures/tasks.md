...
- [ ] 5.1 `docs/mobile/releases/03-first-preview.md` — the signing row (line 119) and the
...
- [ ] 9.1 `web/app/mockups/calendar-confetti/page.tsx` line 125 — a greeting heading in mockup
...
- [ ] 10.1 Build the tree scanner in the run's scratch directory. **Never write the pattern list,
...
- [ ] 10.2 Walk every tracked file (`git ls-files`), matching each pattern against the file's whole
      content **and** against its path. Report per-file occurrence counts.
- [ ] 10.3 The result is **exactly 5 paths carrying 7 occurrences**: the two archived About-screen
...
- [ ] 10.4 Confirm no protected string appears in the diff, in any commit message, or in the pull
...
- [ ] 10.5 Run the pre-publication scan before opening or editing the pull request, as usual. Note
...
- [ ] 11.1 Tell TIM-473 the residue this change leaves for the committed baseline: **5 paths / 7
...
- [ ] 11.2 Tell TIM-470 that the `mobile-about-screen` carve-out under-covers its own intent by one
...
- [ ] 11.3 Tell TIM-468 the gate consequence: a branch touching any of the 5 residual paths reports
...
- [ ] 11.4 Confirm to the Founding Engineer that the carve-out on all-`added: false` reports can be
