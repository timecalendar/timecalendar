...
- [x] 1.1 Add a pure UTC calendar-date helper with named 12-month past/future bounds,
...
- [x] 1.2 Add the structural ADE iCal URL renamer using `URL`/`URLSearchParams`; verify
...
- [x] 1.3 Replace the generic 2000–2038 `nbWeeks` expansion with webcal conversion followed
  by the bounded ADE renamer; verify the legacy range is no longer emitted.

## 2. Strategy composition and sync behavior

- [x] 2.1 Make `FetchService` apply the generic strategy exactly once when no school strategy
...
- [x] 2.2 Add real registered-strategy regression cases for generic/unmatched ADE,
...
- [x] 2.3 Extend calendar-sync service coverage through the real URL-transformation seam to
...
- [x] 2.4 Retain or extend the Lyon 1 batch-sync cadence proof: creation plus repeated sync
...
- [x] 3.1 Update `docs/mobile/architecture-book/calendar.md` at the sync boundary with the
...
- [x] 4.1 Run the focused renamer, fetch-service, representative-school, calendar-sync, and
...
- [x] 4.2 Run server local-green checks (`npm run build`, `npm run lint`, and `npm test --
  --runInBand`), confirm no unintended OpenAPI/generated-client diff, and resolve every
  failure in scope.
- [x] 4.3 Run `openspec validate normalize-ade-export-date-windows` and confirm the final
