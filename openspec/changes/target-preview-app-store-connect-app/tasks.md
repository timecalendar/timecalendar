...
- [ ] 1.1 In `mobile/eas.json`, replace only `submit.preview.ios.ascAppId` with the string
...
- [ ] 2.1 Update `mobile/app.config.test.ts` so preview and production submit expectations are
...
- [ ] 3.1 Update `docs/mobile/architecture-book/eas.md` and
...
- [ ] 3.2 Update `mobile/EAS.md`, `docs/mobile/releases/03-first-preview.md`, and
...
- [ ] 4.1 Run `cd mobile && npm test -- --runInBand app.config.test.ts` and the root-level direct
...
- [ ] 4.2 Run `openspec validate target-preview-app-store-connect-app --strict`, inspect the final
...
- [ ] 4.3 Push implementation commits to the existing draft PR and use green GitHub mobile CI at
