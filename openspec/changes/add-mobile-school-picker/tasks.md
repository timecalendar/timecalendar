...
- [ ] 1.1 `src/features/school-selection/data/types.ts` — add `code: string` to `SchoolListItem`
  (the generated `SchoolForList` already carries `code`). Keep the shape minimal (only what the screen
  renders + what search needs).
- [ ] 1.2 `src/features/school-selection/data/queries.ts` — `toSchoolListItem` projects `school.code`
  into the domain shape. No other change to the read seam (it stays the only generated-hook import
  site, B-1).
- [ ] 1.3 `src/features/school-selection/data/search.ts` — a pure `normalize(s: string)`
...
- [ ] 1.4 `src/features/school-selection/data/index.ts` — export `schoolMatches` (and `normalize` if
  the test imports it) from the `data/` sub-barrel.
- [ ] 1.5 `src/features/school-selection/data/search.test.ts` (90% gate): asserts diacritic-insensitive
...
- [ ] 2.1 `src/features/school-selection/ui/school-picker-screen.tsx` — the `useMemo` filter calls
...
- [ ] 2.2 Extend `src/features/school-selection/ui/school-picker-screen.test.tsx`: typing an
...
- [ ] 3.1 `src/features/school-selection/ui/school-group-picker-screen.tsx` — replace the
...
- [ ] 3.2 Extend `src/features/school-selection/ui/school-group-picker-screen.test.tsx` — extend the
...
- [ ] 4.1 Add flat dotted keys to **both** catalogs: `onboarding.group.confirm` (the confirm action
...
- [ ] 5.1 `mobile/.maestro/onboarding.yaml` — keep the welcome → CTA → live `GET /schools` →
...
- [ ] 6.1 **Architecture** — follows the Architecture Book: golden-path layered module (pure search
...
- [ ] 6.2 **Types** — `npx tsc --noEmit` clean in `mobile/`; no unjustified `any`.
- [ ] 6.3 **Lint** — `npm run lint` clean (`--max-warnings 0`): no hardcoded strings, a11y props on the
  new leaf toggle + confirm control, no parent-relative imports, import order, feature boundaries.
- [ ] 6.4 **Unit/component tests** — the §1.5 search helper green under the 90% gate; the §2.2 + §3.2
...
- [ ] 6.5 **E2E** — `mobile/.maestro/onboarding.yaml` extended per §5 (live read preserved; optional
  stable search; no fixture-dependent group step); runs on iOS + Android via `ci-mobile-e2e.yml`
  (on-demand) — on-device run is §7.
- [ ] 6.6 **i18n** — zero hardcoded strings (lint); FR + EN complete (`tsc` bidirectional parity). ✅ §4.
- [ ] 6.7 **Accessibility (automatable half)** — a11y lint passes; each leaf toggle declares a role +
...
- [ ] 6.8 **Native correctness (automatable half)** — RN-core primitives + Expo Router `dismissTo`
  (the platform's nav — R-3); the on-device feel review is §7 (inbox).
- [ ] 6.9 **Performance** — the search filter is a normalized substring scan over the already-loaded
  list (no new query); the tree render is unchanged. No new jank surface; on-device perf eyeball → §7.
- [ ] 6.10 **Observability** — ➖ **N/A with reason**: the selection commit is synchronous MMKV writes
  ...
  DoD audit trail.
- [ ] 6.11 **Product analytics** — **deferred-with-owner**: a "school selected" / "groups confirmed"
...
- [ ] 6.12 **Theming / light-dark** — the leaf selected-state + confirm control render
...
- [ ] 6.13 **Documentation** — Architecture Book + changelog + ADR 016 + golden-path note (§8). This §6
  + §7 is the DoD audit trail.
- [ ] 6.14 **Native config sanity** — **no native module / no `app.config.ts` change** and **no new
...
- [ ] 8.1 **ADR** `.claude/rules/mobile/decisions/016-school-group-multi-select-commit.md` (multi-select
...
- [ ] 8.2 **Architecture Book — `features.md`** — update the "School selection" section: the group step
  ...
  Pointer style (R-1).
- [ ] 8.3 **Architecture Book — `data.md`** — note the school-selection `data/search.ts` pure helper as
  the data-layer's pure-logic example (normalize + match, 90%-gated). Pointer style.
- [ ] 8.4 Append an entry to `.claude/rules/mobile/architecture-changelog.md` (Live, 2026-06-15):
...
- [ ] 8.5 Update `.claude/rules/mobile/golden-path.md` if a "closest reference" shifts (the
...
- [ ] 8.6 Update `docs/react-native-migration/01-roadmap/03-onboarding-and-sources.md` step 2:
...
- [ ] 8.7 **Append** the §7 on-device DoD items to the existing
...
- [ ] 9.1 `npx tsc --noEmit` clean in `mobile/`.
- [ ] 9.2 `npm run lint` clean in `mobile/` (`--max-warnings 0`).
- [ ] 9.3 `npm test` green in `mobile/` (the §1.5 search helper at 90%; the §2.2 + §3.2 screen tests at
  the 70% floor; the K-3 gate green; `jest.config.js` unchanged).

## 10. Validate

- [ ] 10.1 `openspec validate add-mobile-school-picker --strict` passes.
