...
- [x] 1.1 In `mobile/`, install the two TanStack Query persister packages **version-locked to the
...
- [x] 2.1 Add a `Storage`-shaped export to `mobile/src/storage/index.ts` (e.g. `mmkvQueryStorage`) over
...
- [x] 2.2 Add `mobile/src/storage/storage.test.ts` coverage (extend the existing seam test): the adapter
...
- [x] 3.1 Set the **earned query policy** on the `QueryClient`. Decide where it's cleanest: either add
...
- [x] 3.2 Build the persister config — `src/features/school-selection/data/persist.ts` (or
...
- [x] 3.3 Swap the provider in `src/app/_layout.tsx`: replace `<QueryClientProvider client={queryClient}>`
...
- [x] 4.1 `data/types.ts`: the small domain shapes the screens consume (e.g. a `SchoolListItem`
  ...
  store persists (R-2).
- [x] 4.2 `data/queries.ts`: `useSchools()` wrapping `useSchoolControllerFindSchools()` →
...
- [x] 4.3 `data/persist.ts` (if placed here per 3.2) — the persister + `persistOptions` (3.2). If placed
  in `src/api/`, note it in the barrel/imports instead.
- [x] 4.4 `data/index.ts` (or fold into the feature barrel §6.4): re-export `useSchools`,
...
- [x] 5.1 `store/types.ts`: the persisted selection shape + flat namespaced keys (e.g.
...
- [x] 5.2 `store/store.ts`: imperative `getSelection()` / `selectSchool(schoolId)` /
...
- [x] 5.3 `store/hooks.ts`: a reactive `useSelectedSchool()` (and group) over `@/storage`'s
  `useStoredString`, so consumers re-render when the selection changes (mirroring `useThemePreference`).
- [x] 5.4 `store/index.ts` (or fold into the feature barrel).

## 6. Feature barrel — no import cycle

- [x] 6.1 `src/features/school-selection/index.ts`: re-export the public surface — `useSchools`,
...
- [x] 7.1 Add flat dotted `onboarding.*` keys to **both** `en.json` and `fr.json` (FR/EN parity is
  ...
  natural FR + EN copy.
- [x] 7.2 **Remove** the orphaned `schools.*` keys from both catalogs (the harness screen is deleted —
...
- [x] 8.1 Presentational screens under `src/components/onboarding/` (70% floor, behavior-tested):
...
- [x] 8.2 Thin routes under `src/app/onboarding/`:
...
- [x] 8.3 Register the group as a root `Stack` sibling: in `src/app/_layout.tsx` add
...
- [x] 8.4 Profile entry control: in `src/app/(tabs)/profile.tsx` add an accessible "Choose your school"
...
- [x] 9.1 Delete `mobile/src/components/schools-screen.tsx` and `mobile/src/components/schools-screen.test.tsx`.
- [x] 9.2 Delete `mobile/src/app/schools.tsx` and remove `<Stack.Screen name="schools" />` from
  `src/app/_layout.tsx`.
- [x] 9.3 Delete `mobile/.maestro/schools.yaml` (replaced by `onboarding.yaml`, §11).
- [x] 9.4 Remove the orphaned `schools.*` i18n keys (§7.2) — confirm `tsc` parity + lint stay green
  after removal (no dangling `t("schools.")`).
- [x] 9.5 Confirm nothing else references the deleted surface (grep `schools-screen`, `app/schools`,
  `schools.yaml`).

## 10. CI proof tests (R-1)

- [x] 10.1 `src/features/school-selection/data/queries.test.ts` (90%): mock at the **`customFetch`
...
- [x] 10.2 `src/features/school-selection/data/persist.test.ts` (90%): `shouldDehydrateQuery` returns
...
- [x] 10.3 `src/features/school-selection/store/store.test.ts` (90%): round-trip the selection through the
...
- [x] 10.4 `src/features/school-selection/store/hooks.test.ts` (90%): `useSelectedSchool()` reflects the
  current selection and re-renders reactively when `selectSchool`/`selectGroup` writes (mirror the
  settings `hooks.test.ts`).
- [x] 10.5 `src/components/onboarding/school-picker-screen.test.tsx` (70%): renders rows from a mocked
...
- [x] 10.6 `src/components/onboarding/school-group-picker-screen.test.tsx` (70%): renders the group tree
  from a mocked `useSchoolGroups`; selecting a leaf calls the selection store (persists the group value).
- [x] 10.7 Confirm the **K-3 coverage gate stays green**: `src/features/school-selection/**` meets 90%
...
- [x] 11.1 Create `mobile/.maestro/onboarding.yaml` mirroring the (deleted) `schools.yaml` /
...
- [x] 12.1 **Architecture** — follows the Architecture Book (Data-layer seam + the now-set query policy
  ...
  nav API.
- [x] 12.2 **Types** — `npx tsc --noEmit` clean in `mobile/`; no unjustified `any` (the generated DTOs +
  the persister generics cover it).
- [x] 12.3 **Lint** — `npm run lint` clean (`--max-warnings 0`): no hardcoded strings, a11y props on
...
- [x] 12.4 **Unit/component tests** — the §10 proofs green; the **90% logic glob**
...
- [x] 12.5 **E2E** — `mobile/.maestro/onboarding.yaml` (§11) is the real-round-trip flow replacing
  `schools.yaml`; runs on iOS + Android via `ci-mobile-e2e.yml` (on-demand) — confirmed in §13 (on-device).
- [x] 12.6 **i18n** — zero hardcoded strings (lint); FR + EN complete (`tsc` bidirectional parity); the
  orphaned `schools.*` keys removed. ✅ via §7.
- [x] 12.7 **Accessibility (automatable half)** — a11y lint passes; every interactive control declares a
...
- [x] 12.8 **Native correctness (automatable half)** — RN-core list/pressable primitives + Expo Router
...
- [x] 12.9 **Performance** — the school list uses `FlatList` with keyed rows; the persister keeps the
...
- [x] 12.10 **Observability** — ➖ **N/A with reason**: the read path's failures surface as TanStack
...
- [x] 12.11 **Product analytics** — **deferred-with-owner** (not silent N/A): a "school selected" /
...
- [x] 12.12 **Theming / light-dark** — the onboarding screens render scheme-appropriate `@/theme`
...
- [x] 12.13 **Documentation** — Architecture Book updates + changelog (§14); ADR 013 (§14); the
...
- [x] 14.1 **ADR** `.claude/rules/mobile/decisions/013-query-persister-and-policy.md` (copy this
...
- [x] 14.2 **Architecture Book — "Data layer → Query runtime"**: update the "query policy deliberately
...
- [x] 14.3 **Architecture Book — add a "School selection" section** (under the Phase-2 features,
...
- [x] 14.4 Append an entry to `.claude/rules/mobile/architecture-changelog.md` (Live section): School
  ...
  cache offline.
- [x] 14.5 **Update `.claude/rules/mobile/golden-path.md`** — the "closest real references today" list:
...
- [x] 14.6 Update `docs/react-native-migration/01-roadmap/02-pattern-establishment.md` step 3: Feature C
...
- [x] 14.7 Create `docs/react-native-migration/inbox/2026-06-14-school-selection-dod-manual.md` (mirror
...
- [x] 15.1 `npx tsc --noEmit` clean in `mobile/`.
- [x] 15.2 `npm run lint` clean in `mobile/` (`--max-warnings 0`).
- [x] 15.3 `npm test` green in `mobile/` (all §10 proofs + existing suites; the K-3 gate green, 90%
...
- [x] 16.1 `npx openspec validate add-mobile-school-selection --strict` passes.
