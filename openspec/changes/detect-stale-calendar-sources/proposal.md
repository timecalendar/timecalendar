...
- Add server classifier/contract tests, mobile data/UI/i18n/accessibility tests, and a
  labelled Maestro recovery flow for CI. Record simulator/device-only visual and
  accessibility checks as a `(HUMAN: …)` migration inbox note.
...
- Mobile calendar sync data/store seam, Calendar status UI, calendar-source management UI,
  typed French/English translations, tests, and Maestro flow.
