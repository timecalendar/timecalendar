...
- [ ] 1.2 Run `git merge-tree --write-tree HEAD origin/main`, inventory textual conflicts plus
...
- [ ] 2.1 Inspect the integrated call graph and prove `CalendarSyncService` retains the persisted
...
- [ ] 2.2 Run the focused ADE renamer/fetch and calendar-sync creation/resync tests that prove
...
- [ ] 2.3 Run the source-health helper/service/controller suites that prove retired AMU and
...
- [ ] 3.1 Compare both parents and preserve `docs/mobile/architecture-book/calendar.md` guidance
...
- [ ] 3.2 Preserve canonical `calendar-source-health`, `server-ade-export-window`, and
...
- [ ] 3.3 Assert `openapi/openapi.json` and `mobile/src/api/generated/` retain the complete
...
- [ ] 4.1 Prove `git diff origin/main -- mobile/app.config.ts` is empty and the integration work
...
- [ ] 4.2 Run the smallest focused server suites from section 2, `git diff --check`, strict
  validation with
...
- [ ] 4.3 Mark completed tasks with evidence, archive the one-off change using
