...
- [x] 1.1 Add closed TypeScript types for calendar-import classification, allowlisted
  ...
  model.
- [x] 1.2 Implement a table-driven, value-blind classifier over normalized school code,
  ...
  unknown fallback.
- [x] 1.3 Add supported-export precedence tests for every cataloged host where a valid
...
- [x] 1.4 Extend the Rennes strategy matcher to `planning.univ-rennes.fr` and add focused
...
- [x] 1.5 Refactor the iCal fetch boundary just enough to expose bounded status-family,
...
- [x] 2.1 Add the Swagger-documented `CalendarImportErrorDto` with only constant code,
...
- [x] 2.2 Extend controller/sync integration tests for unsupported preflight, provider
...
- [x] 2.3 Regenerate `openapi/openapi.json` with `cd server && npm run
...
- [x] 3.1 Change `CalendarFailure` and its repository API to accept only bounded school
...
- [x] 3.2 Add a `server/src/migrations/` migration that irreversibly scrubs legacy raw URL
...
- [x] 3.3 Add a PostgreSQL-backed `up → down → supported-forward` migration proof seeded
...
- [x] 3.4 Remove source-domain/URL-derived metric labels and emit only allowlisted bounded
...
- [x] 3.5 Redact calendar-create request/response bodies from the mobile development API
...
- [x] 4.1 Replace the temporary `"Dev import"` calendar-create context with a typed add
...
- [x] 4.2 Add a total pure type guard/mapper from `ApiError<unknown>` to the feature-domain
...
- [x] 4.3 Replace recording of the original import error with a newly constructed
...
- [x] 5.1 Add flat typed EN/FR title, instruction, and action keys for the nine required
  ...
  provider problem.
- [x] 5.2 Replace the generic iCal error block with an accessible recovery panel: resolved
...
- [x] 5.3 Add component tests under both real EN and FR locales for representative Tours
...
- [x] 5.4 Narrow iCal → Feedback route and DTO context to allowlisted classification/help
...
- [x] 6.1 Extend the mail-safe Maestro iCal flow and deterministic E2E server fixture to
...
- [x] 6.3 Update `docs/mobile/architecture-book/data.md`, `features.md`, and `CHANGELOG.md`
  ...
  before proceeding.
- [x] 6.4 Run focused server classifier/fetch/calendar-sync/controller/metrics/repository/
