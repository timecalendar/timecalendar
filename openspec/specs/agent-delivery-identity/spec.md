...
Pins what `docs/agent-dev-environment.md` must say about how an autonomous agent run
authenticates to GitHub: the GitHub App installation identity, the injected short-lived
token and its process scope, the repo-local commit identity, and the credential
operations an agent must never run. The handbook is the only place a cold agent learns
this before touching its own environment, so these requirements exist to keep it true —
describing the live arrangement and nothing else.
