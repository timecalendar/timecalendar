# ios-platform-minimum Specification

## Purpose
Captures the iOS deployment-target minimum for the Flutter app — the lowest
iOS version the app builds against and ships to. The minimum is driven by
the native dependencies in [`firebase-sdk`](../firebase-sdk/spec.md) (and
similar platform-coupled SDKs), and the device-coverage tradeoff is a
product decision recorded here so it stays explicit across upgrades.
## Requirements
