...
class CalendarEvent {
  ...
  static List<CalendarEvent> listFromEvents(List<Event> events) {
    ...
    for (int eventIndex = 0; eventIndex < calendarEvents.length; eventIndex++) {
      CalendarEvent calendarEvent = calendarEvents[eventIndex];
      int startColumn = 0;
      int endColumn = 1;
      int columns = 1;
      ...
      List<CalendarEvent> overlapEvents = [];
      ...
      int maxColumns = 1;
      for (CalendarEvent overlap in overlapEvents) {
      ...
      if (overlapEvents.length > 0) {
        ...
        for (int columnIndex = 0; columnIndex < oldColumns; columnIndex++) {
          ...
          List<CalendarEvent> overlapsOfColumn = overlapEvents.where((ev) {
  ...
  static getOverlapEventsBefore(
      ...
      List<CalendarEvent> overlapEvents) {
    CalendarEvent calendarEvent = calendarEvents[eventsBeforeIndex];

    for (int i = 0; i < searchBeforeIndex; i++) {
      CalendarEvent overlap = calendarEvents[i];
