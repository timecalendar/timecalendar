...
import 'package:timecalendar/modules/calendar/repositories/deprecated_event_repository.dart';
...
class EventsProvider with ChangeNotifier {
  ...
  List<DeprecatedEvent> eventsUidHidden = [];
  List<DeprecatedEvent> eventsNamedHidden = [];
  List<EventByDay> eventsByDay = [];

  HiddenEvent hiddenEvent =
      new HiddenEvent(namedHiddenEvents: [], uidHiddenEvents: []);

  HiddenEventRepository hiddenEventManager = new HiddenEventRepository();
  ...
  Future<void> loadEventsFromDatabase() async {
    var tmpEvents = await DeprecatedEventRepository().getEvents();
    List<DeprecatedEvent?> personalEvents =
        await PersonalEventManager().getEvents();
  ...
  Future<void> fetchAndSetEvents() async {
    try {
      ...
      var eventManager = DeprecatedEventRepository();
  ...
  updateEventNotes() async {
    var numberOfNotes =
        await ChecklistItemRepository().findEventNumberOfNotes();
    ...
    events = List<DeprecatedEvent?>.from(events);
  ...
  List<DeprecatedEvent?> get homeEvents {
  ...
  List<EventByDay> getDayEventsList() {
    ...
    List<DeprecatedEvent?> currentDayEvents = [];
  ...
  List<List<DeprecatedEvent>> getWeekEvents(int weekNumber) {
    ...
    List<List<DeprecatedEvent>> dayEvents = [];
    for (var day = 0; day < 7; day++) {
      ...
      currentDayEvents.addAll(weekEvents
          ...
          .toList());
      dayEvents.add(List<DeprecatedEvent>.from(currentDayEvents));
    }
  ...
  void controlEvent() {
    eventsUidHidden = List<DeprecatedEvent>.from(events);
    eventsUidHidden = eventsUidHidden
        .where((item) => hiddenEvent.uidHiddenEvents.contains(item.uid))
        .toList();

    eventsNamedHidden = List<DeprecatedEvent>.from(events);
  ...
  Future<void> addPersonalEvent(DeprecatedEvent event) async {
  ...
  Future<void> updatePersonalEvent(DeprecatedEvent event) async {
