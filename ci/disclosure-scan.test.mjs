...
test("personal commit metadata passes while the same values in files remain scanned", (t) => {
  const { repo, runGit } = createGitRepo(t, "disclosure-metadata-scope-");
  const token = ["fixture", "personal", "identity"].join("-");
  const address = addr(token, "gmail.com");
  const identity = {
  ...
  const message = `mentions ${token}\n\n${homePath(token)}\n${coAuthor(token, address)}`;
  writeFileSync(join(repo, "README.md"), "base\n");
  runGit("add", "README.md");
  runGit("commit", "--quiet", "-m", "base");
  const base = runGit("rev-parse", "HEAD").trim();

  writeFileSync(join(repo, "README.md"), "ordinary content change\n");
  runGit("add", "README.md");
  commitWithIdentity(repo, identity, message);
  const args = ["--base", base, "--head", "HEAD", "--cwd", repo];
  for (const env of [{}, { DISCLOSURE_PATTERNS: token }]) {
    const { code, output } = runMain(env, args);
  ...
  runGit("add", "README.md");
  commitWithIdentity(repo, identity);
  const { code, output } = runMain({ DISCLOSURE_PATTERNS: token }, args);
  ...
  for (const finding of [
    ...
    FINDING_CLASSES.EMAIL,
    FINDING_CLASSES.HOME_PATH,
    FINDING_CLASSES.CO_AUTHOR,
  ]) {
    assert.ok(output.includes(finding), output);
  }
