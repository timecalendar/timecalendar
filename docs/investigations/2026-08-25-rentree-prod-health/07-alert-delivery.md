...
globally would therefore page for sibling-service-d and sibling-service-a while still providing no ongoing
TimeCalendar detection.

## Evidence

Read-only checks were run on 2026-08-25 at 18:20 UTC against Grafana 11.6.1 in the
`<cluster>` `observability` namespace and against platform `origin/main` at
`edbcbc0a7315b5e38393572483ab1770e3b82742`.

- `terraform/envs/observability/contacts.tf` defines one `email-oncall` contact point,
  an `all-time-temporary-receiver` mute timing from `00:00` to `24:00`, and a catch-all
...
- Grafana's provisioning APIs report the same contact point, mute timing, root policy,
  ...
  source control.
- Grafana reports 22 provisioned alert rules: 18 in the sibling-service-d folder and 4 in the
  sibling-service-a folder. It reports **0** in the TimeCalendar folder.
...
- Active sibling-service-d and sibling-service-a alerts currently resolve to `email-oncall`; the catch-all
...
- The same mute suppresses sibling-service-d and sibling-service-a notifications. A global unmute risks a
...
[
  {
    ...
    "generatorURL": "https://grafana.internal/alerting/list"
  }
]
```
...
| sibling-service-d | `email-oncall`, `all-time-temporary-receiver` |
| sibling-service-a | `email-oncall`, `all-time-temporary-receiver` |
