...
- The contract carries exactly **one path-level `/v1` route**: `PATCH /v1/calendars/{token}` (the token-authorized calendar rename), served by its own `CalendarV1Controller` in the server's existing `CalendarModule`. NestJS global versioning (`app.enableVersioning`) is **deliberately not enabled** — it would apply a default version to every controller, and Flutter release builds in the field call the unversioned paths. Every other calendar route (read by token, create, sync) stays unversioned. An API-wide versioning migration is deferred; nothing here commits to versioning anything else.
