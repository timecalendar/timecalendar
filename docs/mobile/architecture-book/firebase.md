...
- `src/firebase/index.ts` is the **single seam** the app touches — `logEvent` / `logMessage` / `recordError` / `recordUnknownError` / `crashTest` over the modular SDK. Feature code imports `@/firebase`, **never `@react-native-firebase/*` directly** (swappable, behind our own abstraction). Each helper resolves the native instance **lazily** inside its body, so importing the module never touches native code (safe in Jest).
- **Recording a caught value — always `recordUnknownError(error, context)`.** A `catch` binding is `unknown`; `recordUnknownError` owns the `error instanceof Error ? error : new Error(String(error))` narrowing (it delegates to `recordError`) so **no call site re-inlines that ternary** (docs review R-1 — it had drifted to 11 copies). Reach for `recordError(error: Error, …)` only when you already hold a typed `Error`.
...
- **Crashlytics/Analytics need no `import "@/firebase"` in `_layout.tsx`.** RNFirebase **auto-initializes** the native default app from the bundled config files and **auto-installs the global JS exception handler** — so unhandled JS errors reach Crashlytics for free and there is nothing to run at startup for _those_ surfaces. **Messaging is the exception** (ADR [026](./decisions/026-fcm-messaging-seam.md)): its background handler is a JS registration that _must_ run early, so the seam **is** now a side-effect `import "@/firebase"` in `_layout.tsx` (mirroring i18n) — see **Cloud Messaging** below. Do not "fix" that import out.

## OTA bundle identity

ADR [037](./decisions/037-self-hosted-ota-runtime.md) adds bundle identity through the same owned
...
> **First consumer (Phase 06 Ship B, ADR [027](./decisions/027-fcm-subscription-registration.md)).** `getFcmToken` / `onFcmTokenRefresh` / `requestNotificationPermission` / `recordError` now have their first consumer: the subscription-registration seam in **`src/features/notifications/`** (NOT this seam — B-1 bans the generated client here; the registration is a feature, not transport). It reads the token, assembles the `notification-subscription` DTO — including the localization pair read through the feature's effective accessors: `locale` (the app's effective language: settings override, else device detection) and `timezone` (device IANA zone, `Europe/Paris` fallback) — and **PUTs it idempotently** on every prefs change + on token-refresh + on an i18n `languageChanged` + on a device-timezone change. The subscription API is **PUT-only (no GET)**, so the **local MMKV `@/storage` store is the source of truth** — the seam never reads subscription state back from the server; "round-trips" means the PUT succeeds. A failed PUT → `recordError(error, "notifications/subscription")` + an accessible Retry on the prefs screen. The seam's shape/posture is unchanged.

> **Tap-routing dispatcher (Phase 06 Ship C, ADR [028](./decisions/028-fcm-tap-routing.md)).** `onForegroundMessage` / `onNotificationTap` / `getInitialTap` (+ `recordError`) drive `useNotificationTapRouting` in **`src/features/notifications/data/tap-routing.ts`** (mounted in `_layout.tsx` beside `NotificationRegistration`). It decodes the server's v2 data messages (`calendar_changed` with lowercase `type ∈ new | edit | cancel`, and `calendar_digest`) with a **pure** `parseNotificationRoute` and dispatches across the three app states: **foreground → refetch the calendar only (no navigation**, Flutter parity); **background tap + killed/cold-start → refetch then navigate** to the affected event (`new`/`edit` → `/event-details/<uid>`) or the calendar (`cancel` and digests → `/calendar`). The refetch reuses `useSyncCalendars` (the existing orchestrator — no new sync path); the cold-start read runs in a ref-guarded mount effect so the `<Stack>` is mounted before `router.push` (ADR 028 Decision 3). A malformed payload → `recordError(error, "notifications/tap-routing")` + a no-op. The seam's shape is unchanged beyond the two new tap helpers.
>
> **Two cross-feature calls, not one (TIM-399, ADR [049](./decisions/049-activity-trigger-edges-and-failure-isolation.md)).** The dispatcher also fires `void refreshNewestPage({ force: true })` from `@/features/activity`, **beside** `void sync()` and never chained onto it, so the push guarantee survives a sync that fails; the coordinator's single-flight slot collapses the duplicate when the sync also succeeds. Relevance is one shared predicate over `data.action ∈ { calendar_changed, calendar_digest }`, used at all three entrypoints — **the action, not `parseNotificationRoute(…) !== null`**: a `calendar_changed` whose `payload` cannot be decoded is still a real calendar change, so routing declines to navigate while Activity still refreshes. Neither call's failure reaches the other, and neither changes what a notification does.

> **Subscription synchronization has one app-lifetime owner ([D03](../../projects/native-settings/decisions/D03-notification-sync.md)).** The root notification runtime is the sole token-refresh listener and subscription PUT source. Each attempt rebuilds current preferences, token, loaded calendar IDs, effective locale, and display timezone; requests are serialized and newer generations coalesce to one next snapshot. Only dirty/generation bookkeeping persists. Shared pending, prerequisite-waiting, retryable-error, and remotely-acknowledged status survives notification-route navigation. Permission state and actual push delivery remain separate and are not implied by acknowledgment.

- **Six lazy-native helpers**, one per concern, each resolving `getMessaging()` inside its body (same posture as the Crashlytics/Analytics helpers): `requestNotificationPermission` (iOS UNUserNotification prompt + Android 13+ `POST_NOTIFICATIONS` runtime request), `getFcmToken`, `onForegroundMessage` (returns an unsubscribe), `onFcmTokenRefresh` (returns an unsubscribe), and — the two **tap entrypoints** (Phase 06 Ship C, ADR [028](./decisions/028-fcm-tap-routing.md)) — `onNotificationTap` (wraps `onNotificationOpenedApp`, the background→foreground tap; returns an unsubscribe) and `getInitialTap` (wraps `getInitialNotification`, the killed/cold-start tap; a one-shot `Promise<RemoteMessage | null>` at launch). Feature code imports `@/firebase`, **never `@react-native-firebase/messaging` directly**.
...
- CI **cannot** assert an event/crash _arrives_ in the console — that is the **manual on-device step**: Firebase DebugView for the event, the Crashlytics dashboard for the crash. The Maestro e2e asserts a seeded school name; Firebase just must not break app launch.
