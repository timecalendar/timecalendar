...
> **Tap-routing dispatcher (Phase 06 Ship C, ADR [028](./decisions/028-fcm-tap-routing.md)).** `onForegroundMessage` / `onNotificationTap` / `getInitialTap` (+ `recordError`) drive `useNotificationTapRouting` in **`src/features/notifications/data/tap-routing.ts`** (mounted in `_layout.tsx` beside `NotificationRegistration`). It decodes the server's `calendar_changed` data message with a **pure** `parseNotificationRoute` and dispatches across the three app states: **foreground → refetch the calendar only (no navigation**, Flutter parity); **background tap + killed/cold-start → refetch then navigate** to the affected event (`NEW`/`EDIT` → `/event-details/<uid>`) or the calendar (`CANCEL` → `/calendar`). The refetch reuses `useSyncCalendars` (the existing orchestrator — no new sync path); the cold-start read runs in a ref-guarded mount effect so the `<Stack>` is mounted before `router.push` (ADR 028 Decision 3). A malformed payload → `recordError(error, "notifications/tap-routing")` + a no-op. The seam's shape is unchanged beyond the two new tap helpers.
