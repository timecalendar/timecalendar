...
>
> `047` is likewise taken by an open PR (the ephemeral calendar-import draft), not
> free. `048` skipped past both — the next author should re-check open PRs with
> `gh pr diff <N> --name-only | grep decisions/` rather than assuming the highest
> merged number plus one.
