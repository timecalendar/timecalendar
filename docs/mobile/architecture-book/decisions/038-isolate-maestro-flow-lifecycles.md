...
2. No command before the final startup failure has status `FAILED`. This is global: a failed assertion, application command, or interaction remains terminal even if the flow later records another restart boundary.
...
The classifier no longer needs updating when Maestro changes its error text, and it correctly classifies both a failure that emits no error text and a later restart failure after an earlier phase completed successfully. A retry always reruns the entire top-level flow in a fresh Maestro process; it never resumes inside the recorded epoch. The bound, stated plainly: **an application that deterministically fails to launch matches the startup shape too.** It is still reported red — it exhausts all four attempts and the harness exits non-zero with the original status. Retry costs attempts, never correctness. A malformed or unreadable command record is treated as terminal, so the classifier fails closed.
