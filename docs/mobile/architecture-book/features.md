...
| `activity`         | Device-local calendar-log cache, watermark, pagination seam, `/activity` grouped timeline, and Settings unread badge | `@/db`; reactive `useActivityLogs` / `useActivityState`; `activity_logs` / `activity_state`, merged by server log ID; `@/api/generated/calendar-logs` (`POST /v1/calendar-logs/search`, plain function, no TanStack Query, ADR 048); `findAll` from `calendar-sources/data` for held tokens + IDs. The screen calls forced newest refresh, older-page load, and cache-bounded read marking; screen-open/foreground policy remains TIM-399's ownership |
