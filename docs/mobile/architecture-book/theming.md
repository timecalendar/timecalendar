...
- **Status pairs** are documented + computed in the same block: `positive` and `informational` on `background` and `backgroundElement` for light and dark. Each is an AA body-text pair; status colors remain semantic and never substitute for event/calendar colors.
