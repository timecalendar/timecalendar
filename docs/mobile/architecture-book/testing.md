...
- The export-guide repository follows the same seam at its response-aware variant: repository tests
...
- **Calendar T07 resize proof** adds a pure atomic geometry model, clock-anchor/boundary tables,
  geometry-revision cancellation tests, one/five/seven-column component coverage, the singular
  owner repository contract, source-config tests, and disposable iOS/Android generated assertions.
  Host automation does not claim physical rotation, multitasking, gesture feel, or native chrome;
  those remain exact-build owner evidence and the final T28 matrix.
- **Calendar T08 current-time proof** combines deterministic helper tables, a fake-timer clock-hook
  suite, renderer/screen rollover and visibility cases, and
  `calendar-owned-shell.contract.test.ts`. The contract identifies the focus- and AppState-gated
  minute clock as the only calendar timer owner, checks teardown and minute alignment, keeps the
...
- **Export-guide proof is a separately selected exact-head suite.** `run_e2e.sh --suite export-guide`
  discovers only `mobile/.maestro/export-guide/*.yaml`; omission still selects the exact three
  daily journeys. The manual workflow adds production-identity cold-route guards and emits
  same-SHA JSON/Markdown evidence checked by `check-export-guide-evidence.mjs`. Physical VoiceOver,
  TalkBack, width, lifecycle, and low-end results remain explicit `NOT RUN` cells in the migration
  inbox until observed; none are inferred from Jest, syntax checks, or hosted devices.
...
- **Every Maestro attempt is bounded.** The harness terminates a Maestro process group after
  `MAESTRO_ATTEMPT_TIMEOUT_SECONDS` (15 minutes by default; 10 in the iOS proof lane), retains its
  log and command record, and then applies the same structural classifier. This covers a dead
  XCTest transport whose Maestro JVM does not exit while preserving terminal app/assertion evidence.
