...
| Route / surface                             | Current portrait-tablet behavior or problem                                                                                             | Proposed quick win or explicit disposition                                                                                                                                     | Priority | Owning child / workstream          | Width rule                            | Verification evidence                                                                                                  |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ---------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
...
| `/onboarding` welcome carousel              | **Implemented:** the safe-area root stays uncapped while page copy and actions use independent measured readable lanes; the illustration keeps its own cap. | One compact gutter is preserved at 390 points, and page order, controls, accessibility grouping, animation, and reduced-motion behavior remain unchanged.                      | P1       | TIM-501                            | independent `R` regions               | Focused coverage proves both page/action lanes at 390 and 1024, the 342/640-point content widths, every page/control path, and both motion branches. |
| `/onboarding/connect`                       | **Implemented:** prose, optional institution link, and Back/Continue actions share one measured readable lane.                          | The step remains one ordered column inside its existing safe-area owner; trusted-link filtering and navigation behavior are unchanged.                                        | P1       | TIM-501                            | `R`                                   | Focused coverage proves the 1024 readable cap; existing tests preserve optional-link states, URL filtering, external opening, Back, and Continue. |
| `/onboarding/institution-name`              | **Implemented:** the field, validation message, and CTA share the import journey's measured readable lane.                              | The existing keyboard-avoiding and safe-area owners remain in place, with validation, draft writes, and navigation unchanged.                                                  | P1       | TIM-501                            | `R`                                   | Shared AdaptiveContent boundary coverage proves the lane metrics; the screen suite preserves 100-character validation, editing, keyboard lift/resize, and CTA behavior. |
| `/onboarding/programme`                     | **Implemented:** the optional programme field and sticky Continue action share a measured readable lane.                               | The one-column flow retains its native Skip/back chrome, keyboard paths, validation, draft write, and navigation behavior.                                                      | P1       | TIM-501                            | `R`                                   | Shared AdaptiveContent boundary coverage proves the lane metrics; the screen suite preserves input limits, Skip, Continue, sticky CTA, and both keyboard paths. |
| `/onboarding/school`                        | **Implemented:** rows, separators, feature-owned list header/footer, and loading/empty/error/no-result states share one measured standard lane. | The native search header stays full width; query, filtering, selection, theme, status announcements, and routing behavior remain unchanged.                                    | P1       | TIM-501                            | `S`; native header `N`                | Focused coverage proves aligned 1024 lane edges; existing tests preserve populated/search/no-result/loading/error/empty, light/dark imagery, announcements, and navigation. |
| `/onboarding/groups`                        | **Implemented:** the dormant group hierarchy, surrounding states, and confirm action share one measured standard lane.                 | The route remains a one-column hierarchy with no new shipped navigation entry; selection, expansion, persistence, and retry behavior are unchanged.                            | P2       | TIM-501                            | `S`                                   | Focused coverage proves the 1024 standard cap; existing tests preserve branch expansion, selection, guarded confirm, loading, empty, error, and retry states. |
| `/onboarding/import`                        | **Implemented:** the QR and manual-iCal choices share one measured readable lane.                                                       | Both accessible actions remain vertically ordered and route to the existing import screens; the route owns no permission, validation, create, or retry logic.                  | P1       | TIM-501                            | `R`                                   | Shared AdaptiveContent boundary coverage proves the lane metrics; focused tests preserve labels, both navigation targets, and the orchestration-only boundary. |
| `/onboarding/ical-url`                      | **Implemented:** introduction, field, actions, pending/error content, reporting, and recovery controls share one measured readable lane. | Existing keyboard ownership, validation, durable import, retry/reporting, and dismiss/back behavior remain unchanged.                                                          | P1       | TIM-501                            | `R`                                   | Focused coverage proves the 1024 readable cap; existing tests preserve valid/invalid imports, failure/retry/reporting context, and direct-route dismissal. |
| `/onboarding/qr-scan`                       | **Implemented:** the camera remains full bleed while permission, viewfinder, guidance, import, recovery, and alternate-action content use measured readable lanes. | The fixed-square viewfinder remains undistorted, and permission, scan exclusion, retry, re-arm, manual-import, and dismissal behavior are unchanged.                            | P1       | TIM-501                            | `F` camera + `R` overlay              | Focused coverage proves the full-bleed camera and bounded 1024 overlay; existing tests cover every permission/import/recovery state and late/rapid callback guards. |
| `/user-calendars` and rename dialog         | **Implemented:** loaded/loading/empty list content uses one measured standard lane, and rename content uses a readable lane inside the existing modal. | Native list/header/modal ownership and add, visibility, menu, rename, delete, failure, safe-area, and accessibility behavior remain unchanged; TIM-502 verified Settings entry integration. | P1       | TIM-501 owner; TIM-502 integration | `S` list + `R` dialog                 | Focused coverage proves 1024 list and rename lane bounds; existing suites preserve empty/list/header/add, optimistic visibility, menu, rename validation/retry, delete, and failure behavior. |
