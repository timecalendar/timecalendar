...
- Which Postgres, and where does it live? Is it reachable from `<cluster>`, or is it on
  the VPS box behind the network boundary?
...
`40-observability-spaces` / `50-sibling-service-a-spaces` / `60-password-manager-spaces` files:
...
SealedSecret** — same as the existing DO Spaces keys for the password manager.
...
- **The README's existing gotcha gets worse.** `terraform plan` in `envs/infra` evaluates *all*
  resources, so today a VPS-only change already fails without DO Spaces credentials. Once
...
entire mechanism. It belongs with the store signing keys: **the password manager**, plus a GitHub
...
2. **Postgres** (§6.6): which server, and is it reachable from `<cluster>`? Is it in an
   existing backup job?
