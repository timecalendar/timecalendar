...
| --- | --- | --- |
| Plan / infra | Production, $199/mo (50k MAU included) | Go pod in `<cluster>` |
...
| 1b | **Argo**: namespace `timecalendar-ota` on `<cluster>` + Argo Application (upstream chart + our values), xprem in **control-plane mode** against a new database on the TimeCalendar production DO Postgres cluster, R2 key and `DATABASE_URL` as SealedSecrets (§8.5–8.6) | ¾ day |
