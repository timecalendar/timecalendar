...
`timecalendar-ota`), re-seal, merge, delete the old key. Sealing runs on `dev-host` — it has
...
file (e.g. `dev-host:~/.config/timecalendar-ota/r2.env`), never chat. The sealing command:
...
`kubernetes/clusters/<cluster>/20-apps/timecalendar-ota/env-sealed-secret.yaml`
...
- The **private key** (`codesigning/keys/`) goes into **the password manager**, alongside the store
...
| dev-host sealing capability | `kubectl` v1.33.4 + `kubeseal` 0.18.1 installed in `dev-host:~/.local/bin`; `paperclip-agent-kubeseal` Role (kube-system, `services/proxy` scoped to the controller Service) applied and committed; verified end-to-end from dev-host: cert fetch + seal both work |
