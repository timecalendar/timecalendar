...
| --- | --- | --- |
| `ota.timecalendar.app` | The xprem server (manifests) — an ingress on `<cluster>` | **We do**, with `cloudflare_dns_record` |
...
a VPS-only change fails without the DO Spaces credentials. Adding Cloudflare makes that
...
`<cluster>` over the private network, and DO's managed backups already cover it. **No new
