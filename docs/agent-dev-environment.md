...
| **gh CLI** | authenticated as **`samuelprak`** (admin/push) | Git protocol is **SSH**; the remote is `git@github.com:timecalendar/timecalendar.git` (a host alias in `~/.ssh/config`). A second account `other-human-login` is also logged in — ensure `samuelprak` is the *active* one (`gh auth switch`). |
