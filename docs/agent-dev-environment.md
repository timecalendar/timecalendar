...
| **gh CLI**       | authenticated as **`samuelprak`** (admin/push)                                                                                                     | Git protocol is **SSH**; the remote is `git@github.com:timecalendar/timecalendar.git` (a host alias in `~/.ssh/config`). A second account `other-human-login` is also logged in — ensure `samuelprak` is the _active_ one (`gh auth switch`). |
...
```bash
# 0. Toolchain: Node 24.13.0, Docker, gh as samuelprak (SSH), JDK, Flutter at $HOME/flutter
