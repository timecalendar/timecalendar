...
- **`gh auth status` prints `Git operations protocol: ssh`** under the bot entry, and may
  list other stored accounts marked `Active account: false`. The protocol line is read
  from `gh`'s own config; it does not describe the remote (HTTPS) and does not affect
  how pushes authenticate. Any stored account is inactive and must stay that way.
...
```bash
# 0. Toolchain: Node 24.13.0, Docker, gh as paperclip-timecalendar[bot] (GitHub App token, HTTPS), JDK, Flutter at $HOME/flutter
