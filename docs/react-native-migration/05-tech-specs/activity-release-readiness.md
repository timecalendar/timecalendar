...
| Server image CI artifact        | Run `34194654606`; tar SHA-256 `02d514bd10a17964ffc03e45f160c68adf730df1322e4e8fdb84376150b371f4`                                                                    |
...
| Immutable environment telemetry window | Bounded negative queries against the exact isolated candidate image           | 19 metric series, 105 log records, and 328 traces; each sink returned `token=0 calendar=0 log=0 cursor=0 content=0` | Zero sensitive-category matches                     | [TIM-532](/TIM/issues/TIM-532)   | PASS    |
...
| Previous mobile release against candidate server | Candidate-bound compatibility exercise                | Flutter `3.1.0+134` at app tree `c1ceed8fcc00889a8226e0f72a482aa65f2841ee`, client blob `564626f462f3e94078539bb91d6df7e44269c8fc`: unversioned array 200; v1 envelope 200 | Valid legacy request remains compatible               | [TIM-532](/TIM/issues/TIM-532)       | PASS    |
...
The bounded candidate exercise left its isolated pod Ready with zero restarts and returned 200 from
`GET /health`; migrations and calendar-sync fan-out were disabled. Its rollback proof passed: the
candidate never replaced the normal service, temporary resources and image references were removed,
and the normal preproduction server remained healthy at 1/1 after cleanup.
