...
Measurements are populated only from this candidate run. `PENDING` is not a releasable verdict.
...
| G9   | Smooth cached scrolling on supported devices                        | Maestro selector/wrapper proof green; exact-head Android/iOS CI pending                 | Pending exact-head native jobs; physical passes are non-blocking inbox evidence                                                                     | PENDING |
...
- React Native: TypeScript and lint passed; 145 suites / 1,176 tests passed with coverage gates;
  ...
  are green.
- Native CI: exact-head Android and iOS simulator jobs are pending after the `run-e2e` label and
  readiness commit are pushed. Physical-device follow-up is listed in
...
**PENDING** — evidence collection is in progress. Only `GO` authorizes creation of the separately
governed rollout ticket.
