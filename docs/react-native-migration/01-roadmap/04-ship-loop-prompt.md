...
```
Autonomously complete Phase 4 of the RN migration (docs/react-native-migration/01-roadmap/04-calendar-core.md): the calendar core — the timeline, sync, offline cache, read-only event details, home. This is the LARGEST, HIGHEST-RISK phase in the whole migration. You are FULLY AUTONOMOUS — no human approval for any command (run simulators/emulators, tests, benchmarks, merge PRs, push, all of it). For the SPIKE you MAY write and drop throwaway code yourself (experiments, benchmarks, prototypes — that is the point of a spike). For every real SHIP you CONDUCT; you do not write production code yourself — every unit of shippable work is delegated to sub-agents per the /ship pipeline. Adhere to the Architecture Book (docs/mobile/architecture-book/architecture.md + topical files) and pass the full Definition of Done for every feature.
...
1. ORIENT: `git fetch origin`, check `git log origin/main` + `openspec/changes/archive/` to see how far the phase has progressed. Read the roadmap doc + check whether the spike ADR exists yet (decisions/ in docs/mobile/architecture-book/) — its existence is how you know the gate has been passed.
