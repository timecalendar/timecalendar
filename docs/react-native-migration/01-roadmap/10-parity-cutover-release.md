...
2. **Maestro parity suite** — e2e flows mirroring the Flutter `integration_test` regression set: onboarding (school/QR/iCal), calendar render/scroll, personal-event CRUD, hide-event, notification tap-through, assistant, **and the data migration**. Both-platform runtime health comes from relevant daily runs and deliberate manual release-candidate SHA dispatches; ordinary feature commits do not require their own native run.
...
- Maestro parity suite health demonstrated on both platforms for the release-candidate SHA, including migration-from-Flutter; this release-hardening evidence does not turn native E2E into a per-feature gate.
