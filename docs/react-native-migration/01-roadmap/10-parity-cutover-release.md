...
2. **Native smoke + release-candidate parity** — the daily/manual Maestro pack is limited to three durable journeys: fresh-user listed-school/programme URL import with a real backend round trip, personal-event lifecycle, and subscribed-calendar visibility. Release-candidate acceptance remains broader and human-directed, covering the Flutter regression set including QR, detailed calendar interaction, notifications, assistant, **and data migration**. Both-platform runtime health comes from relevant daily runs and deliberate manual release-candidate SHA dispatches; ordinary feature commits do not require their own native run (ADR [057](../../mobile/architecture-book/decisions/057-three-journey-native-smoke-budget.md)).
...
- The three-journey native smoke pack is healthy on both platforms for the release-candidate SHA, and broader human exploratory parity—including migration from Flutter—is recorded; this release-hardening evidence does not turn native E2E into a per-feature gate.
