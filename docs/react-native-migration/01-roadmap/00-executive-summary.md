...
> **What this is:** the progressive, end-to-end plan for _how we build the RN app_ — every phase, in dependency order, from empty repo to store cutover. Rough by design; each phase gets a real task breakdown only when we reach it (we earn detail, we don't front-load it).
>
...
| ------ | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **01** | [Foundation (walking skeleton)](./01-foundation.md)             | Repo alive: monorepo, EAS, CI, lint + first custom rules, test + Maestro harness, i18n, Firebase, storage seams, the 5 living artifacts, **splash** through full DoD on both platforms. | Getting cross-cutting infra _right_ (it's copied forever).                                     |
...
| **04** | [Calendar core](./04-calendar-core.md)                          | The vendor-backed core shipped in source; the day/week surface is now reopened as the [owned renderer project](../../projects/owned-calendar-renderer/README.md) before launch.         | Owned dense-timeline correctness, accessibility, and release-device performance — the #1 risk. |
...
- **Storage schema ↔ migration compatibility:** when we design the MMKV/SQLite schemas (Phases 02/03/05), we design them so the [Phase 09](./09-data-migration.md) importer can target them cleanly. The importer is _built_ late; its _target shape_ is considered early.
...
- **Owned Calendar renderer:** Phase 04's 2026-06 calendar-kit result remains useful current-system
  evidence, but it is no longer the intended launch renderer. The canonical replacement project is
  at the product-approval gate; architecture and delivery planning have not started.

## What would make me say "slow down"

Nothing structurally — but two standing risks to watch: (1) the full-parity bar means **no user feedback until cutover**, so dogfooding discipline is the safety valve; (2) **the owned Phase 04 renderer is load-bearing and uncertain** and may dwarf several other phases. Product, architecture, implementation, and release evidence must pass their separate gates before cutover.
</content>
