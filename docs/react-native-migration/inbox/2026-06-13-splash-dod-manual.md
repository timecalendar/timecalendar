...
- **Why:** Same — both platforms are reviewed against the _platform_, not the Flutter app.
...
- **How to verify:** Manually dispatch `ci-mobile-e2e.yml` with the ref/SHA under test, or use
  the next relevant daily health run. The flow asserts a seeded school renders.
