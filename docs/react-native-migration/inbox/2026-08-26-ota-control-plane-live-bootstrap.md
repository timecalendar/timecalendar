...
install -d -m 700 -o dev -g dev $HOME/.config/timecalendar-ota
umask 077
kubectl get secret -n timecalendar-ota timecalendar-ota-env-secret \
  -o jsonpath='{.data.ADMIN_PASSWORD}' \
  | base64 -d > $HOME/.config/timecalendar-ota/admin-password
chown dev:dev $HOME/.config/timecalendar-ota/admin-password
chmod 600 $HOME/.config/timecalendar-ota/admin-password
```
...
- This agent's `cluster-paperclip` service account cannot read the namespace, pods, or
